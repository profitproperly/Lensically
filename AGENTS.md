...
- `getOperatorStartupContext` is the only unguarded startup tool. Before every later Lensically tool call, call `guardLensicallyCall` with the intended tool and exact JSON arguments, use its normalized arguments, and include the signed `execution_guard`. The dispatcher rejects missing or mismatched guards, schema-invalid payloads, and known-bad routes before continuity, handlers, or external requests. Direct tools and wrappers that reach the same handler remain aliases, not fallback routes.
- For ChatGPT MCP work, fresh sessions call `getOperatorStartupContext` first and then `guardLensicallyCall` before every other tool, including key selection, continuity, engineering, admin, account, and recovery-adjacent main-MCP calls. Never attempt the real call first and guard or reroute afterward.
- Fresh MCP key selection uses stateless app-compatible enforcement: call `selectOperatorKey`, show only its exact four-line handshake, wait for explicit owner approval, and call `confirmOperatorProceed` with the selected key. `confirmOperatorProceed` automatically restores canonical persisted schedule, workflow, production-day source claims, active review batches, the active account-autonomy profile, and pending owner-ratified account decisions; never ask resume or start fresh. Every later account-scoped call must include `proceed_confirmed=true`, `continuity_loaded=true`, and a stable `operation_id` for mutations. Routine repository work, testing, workflow repair, deployment, runtime verification, routing, MCP infrastructure, and universal engineering improvements run under `operator-engineering-authority-v1`: full-discretion, outcome-bound, and without owner proposals or numerical tool budgets. Account content, scheduling, strategy, destructive data actions, ownership changes, and irreversible business decisions remain separately protected. For Manifest, hourly coverage remains a hard operating constraint. Approved content decisions may claim qualified sources without replacement, silently create/reuse and lock source cards, generate and gate drafts, then show only Post 1–4 with Source and Generated post. Approved items schedule into the earliest open hourly slots in item order, and times are reported only after persistence succeeds. Conversation memory is not accepted as workflow or decision state, and no generated continuity handle is passed between tools.
- `operator-execution-policy-v2` resolves a mandatory source-defined known path before every engineering call. Apply proven compact payload, exact-file local search, YAML whole-block indentation, one-callable-alias, bounded polling, and deployment-identity rules before execution rather than after a blocker. A diagnosed failure with an available prevention policy is a regression and the known-bad route must be blocked.
- When a novel reusable engineering blocker is solved, stop the active sequence, promote the fix into execution policy and regression coverage, verify it, record it in engineering audit and operating memory, then resume the original objective automatically. Recovery is reserved for genuinely independent failures, not known-path rediscovery.
