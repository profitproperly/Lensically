...
- Fresh MCP key selection uses stateless app-compatible enforcement: call `selectOperatorKey`, show only its exact four-line handshake, wait for explicit owner approval, and call `confirmOperatorProceed` with the selected key. Immediately after confirmation, before loading account state or creating/resuming workflow work, ask the owner whether to pick up where the persisted workflow left off or start fresh. Do not infer the choice. If the owner chooses resume, load the persisted workflow and continue from its exact checkpoint without redrawing, restarting, or duplicating work. If the owner chooses start fresh, create a new workflow session while preserving the old session and artifacts unless deletion is separately requested. Only then include `proceed_confirmed=true` on the selected account-scoped path. Use the `listMcpTools` bridge when a cached direct schema does not expose the field. Do not rely on `Mcp-Session-Id`; the ChatGPT app does not preserve it across calls.
