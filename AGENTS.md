...
- `getOperatorStartupContext` is the only direct startup tool. Before every later external action, submit only the objective, action intent, and variable inputs to `executeMappedIntent`. The Mandatory Execution Map—not the model—selects the internal tool, route, payload structure, sequence, and fallback. Direct operational and direct guard calls are rejected before lookup.
- For ChatGPT MCP work, fresh sessions call `getOperatorStartupContext` once and then use `executeMappedIntent` for key selection, continuity, engineering, admin, account, GitHub, Cloudflare, scheduler, publishing, and recovery-adjacent main-MCP actions. A known entry is compulsory. Discovery is permitted only when the map returns a signed unknown or stale incident; the original objective remains paused until the verified solution is promoted and activated.
