...
- `operator-execution-policy-v2` is enforced through `guardLensicallyCall`, not advisory prose. Every newly solved reusable blocker must be encoded in `resolveOperatorKnownPath` or guard-side schema normalization, covered by a regression, audited, and recorded in OpsMemory before the interrupted objective resumes.
