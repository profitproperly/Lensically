...
- Before the first tool call, classify the operation and use its canonical plane: engineering control for repository/tests/deploys, operator control for universal MCP administration, operator account for account workflow, and independent recovery for main-MCP health, large-file repair, or deployment-plane failure. Direct tools and wrappers that reach the same handler are aliases, not fallback routes. Never repeat a deterministic failure with identical inputs through another alias.
- For ChatGPT MCP work, fresh sessions must call `getOperatorStartupContext` before engineering, admin, workflow, or account work. Use one callable alias if the direct schema is stale; do not walk multiple same-backend wrappers after a failure.
- Fresh MCP key selection uses stateless app-compatible enforcement: call `selectOperatorKey`, show only its exact four-line handshake, wait for explicit owner approval, and call `confirmOperatorProceed` with the selected key. `confirmOperatorProceed` automatically restores canonical persisted schedule, workflow, production-day source claims, active review batches, the active autonomy profile, and pending model-originated decisions; never ask resume or start fresh. Every later account-scoped call must include `proceed_confirmed=true`, `continuity_loaded=true`, and a stable `operation_id` for mutations. Under an active autonomy profile, the model selects the next priority from evidence and does not ask the owner to invent routine direction. Read-only investigation is autonomous. Before any substantive strategy, content, scheduling, analytics, experiment, workflow, MCP, engineering, or risk mutation, persist a model-originated decision with rationale, evidence, expected outcome, risks, reversibility, an execution plan, exact authorized tools, and a finite per-tool budget; present it for owner approve/reject/revise during the ratified training phase. The dispatcher must block unapproved and over-budget mutations. For Manifest, hourly coverage remains a hard operating constraint, but the model decides whether coverage, analytics, experimentation, or infrastructure is the next priority. Approved content decisions may claim qualified sources without replacement, silently create/reuse and lock source cards, generate and gate drafts, then show only Post 1–4 with Source and Generated post. Approved items schedule into the earliest open hourly slots in item order, and times are reported only after persistence succeeds. Conversation memory is not accepted as workflow or decision state, and no generated continuity handle is passed between tools.
- A diagnosed failure with an available prevention policy is a regression. Block it before execution, record the policy decision, and add realistic fresh-chat regression coverage. Recovery is reserved for genuinely independent failures, not known-path rediscovery.
