...
- Fresh MCP key selection is session-enforced: call `selectOperatorKey`, show only its exact four-line handshake, wait for explicit owner approval, then call `confirmOperatorProceed`. Account-scoped MCP tools and wrappers must remain backend-blocked before confirmation; prompt instructions alone are not sufficient.
