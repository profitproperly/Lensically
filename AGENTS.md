...
- Fresh MCP key selection uses stateless app-compatible enforcement: call `selectOperatorKey`, show only its exact four-line handshake, wait for explicit owner approval, and call `confirmOperatorProceed` with the selected key. `confirmOperatorProceed` automatically restores canonical persisted schedule, workflow, production-day source claims, and any active four-post review batch; never ask resume or start fresh. Every later account-scoped call must include `proceed_confirmed=true`, `continuity_loaded=true`, and a stable `operation_id` for mutations. For Manifest, calendar coverage controls production: find the earliest incomplete future publishing day, ignore past missed slots, claim four random qualified sources without replacement for that production date, silently create/reuse and lock source cards, generate and gate drafts, then show only Post 1–4 with Source and Generated post. Approved items schedule into the earliest open hourly slots in item order, and times are reported only after persistence succeeds. A stable source identity may be claimed only once per production date across chats; cross-day reuse remains allowed. Conversation memory is not accepted as workflow state, and no generated continuity handle is passed between tools.
