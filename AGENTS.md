...
- Fresh MCP key selection uses stateless app-compatible enforcement: call `selectOperatorKey`, show only its exact four-line handshake, wait for explicit owner approval, and call `confirmOperatorProceed` with the selected key. After the owner chooses resume or start fresh, call `resolveContinuationContext` with the exact owner choice. The backend verifies the short-lived proceed record and stores canonical continuity state in D1. Every later account-scoped call must include `proceed_confirmed=true`, `continuity_loaded=true`, and a stable `operation_id` for mutations. Persisted source selections carry `pending`, `linked`, or `skipped` dispositions and an owner-facing workflow sequence. Resume anchors after the highest linked draw for the latest processed sequence, preserves earlier unlinked draws as historical skips, and advances only to the next pending draw; never reopen a skipped gap or assume draw order equals source-card sequence. Start fresh preserves prior sessions and artifacts unless deletion is separately approved. Conversation memory is not accepted as workflow state, and no generated continuity handle is passed between tools.
