...
assert.match(source, /const maxAttempts = retryableRead \? 4 : 1;/);
...
assert.match(patchBranch, /contents_update_uncertain/);
assert.match(patchBranch, /reconciliation_required: true/);
assert.doesNotMatch(patchBranch, /const reconciledFile = await repoFile/);
...
assert.match(source, /executeOperatorAction\", arguments: \{ live_state_token: liveStateToken, execution_descriptor: executionDescriptor, governing_standards_ack: governingAck \}/);
