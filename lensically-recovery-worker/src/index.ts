...
async function toolCall(name: string, args: Record<string, unknown>, env: Env): Promise<Record<string, unknown>> {
  ...
  if (name === "runMainMcpSmoke") {
    ...
    const campaign = await mainMcpRequest(origin, token, 9, "tools/call", { name: "executeLensicallyIntent", arguments: { objective: "Run compact system checks.", intent: "run system checks", inputs: {} } }, sessionId);
    const scheduledToday = await mainMcpRequest(origin, token, 10, "tools/call", { name: "executeLensicallyIntent", arguments: { objective: "List Manifest Mental scheduled posts for the requested date.", intent: "list scheduled posts", inputs: { brand_key: "manifest_mental", date: "2026-07-18", timezone: "America/New_York", proceed_confirmed: true } } }, sessionId);
    const scheduledTomorrow = await mainMcpRequest(origin, token, 11, "tools/call", { name: "executeLensicallyIntent", arguments: { objective: "List Manifest Mental scheduled posts for the requested date.", intent: "list scheduled posts", inputs: { brand_key: "manifest_mental", date: "2026-07-19", timezone: "America/New_York", proceed_confirmed: true } } }, sessionId);
