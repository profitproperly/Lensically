...
async function toolCall(name: string, args: Record<string, unknown>, env: Env): Promise<Record<string, unknown>> {
  ...
  if (name === "runMainMcpSmoke") {
    ...
    const startup = await mainMcpRequest(origin, token, 4, "tools/call", { name: "getOperatorStartupContext", arguments: {} }, sessionId);
    const accountKey = await mainMcpRequest(origin, token, 5, "tools/call", { name: "selectOperatorKey", arguments: { brand_key: "manifest_mental" } }, sessionId);
    const engineeringAccess = await mainMcpRequest(origin, token, 6, "tools/call", { name: "getEngineeringAccessState", arguments: {} }, sessionId);
    const scheduler = await mainMcpRequest(origin, token, 7, "tools/call", { name: "getScheduledPostSchedulerState", arguments: {} }, sessionId);
    const campaign = await mainMcpRequest(origin, token, 8, "tools/call", { name: "runMcpTests", arguments: { segment: "s0" } }, sessionId);
    const scheduledToday = await mainMcpRequest(origin, token, 9, "tools/call", { name: "list_scheduled_posts", arguments: { brand_key: "manifest_mental", date: "2026-07-20", timezone: "America/New_York", proceed_confirmed: true } }, sessionId);
