...
// GitHub GET/HEAD reads use bounded transient retries; mutations remain single-attempt and reconcile separately.
...
const TOOLS: Tool[] = [
  ...
  { name: "runMainMcpSmoke", title: "Run main MCP smoke", description: "Run live Main OAuth, initialization, deployment-scoped session enforcement, the seven-tool five-stage Operator lifecycle, effect-matched token-only post-preparation execution and closure, and runtime identity verification.", inputSchema: { type: "object", properties: {}, additionalProperties: false }, annotations: { readOnlyHint: true, openWorldHint: true } },
];
...
async function toolCall(name: string, args: Record<string, unknown>, env: Env): Promise<Record<string, unknown>> {
  ...
  if (name === "applyRepoTextPatch") {
    ...
    if (content === file.content) return { ok: true, status: 200, path, commit_sha: null, phase: "no_change", write_mode: "git_data_api" };
        const encodedPath = path.split("/").map(encodeURIComponent).join("/");
    const update = await github(env, `/repos/${config.owner}/${config.repo}/contents/${encodedPath}`, {
      method: "PUT",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ message, content: textToBase64(content), sha: file.sha, branch: config.branch }),
    });
    const updateData = update.data && typeof update.data === "object" && !Array.isArray(update.data) ? update.data as Record<string, unknown> : null;
    const updateCommit = updateData?.commit && typeof updateData.commit === "object" && !Array.isArray(updateData.commit) ? updateData.commit as Record<string, unknown> : null;
    const updateCommitSha = typeof updateCommit?.sha === "string" ? updateCommit.sha : null;
  ...
  if (name === "runMainMcpSmoke") {
    ...
    const governingAck = "Autonomy. Efficiency. Prevention. Use the fastest complete route; stop on every blocker, fix the root cause, record it, prevent recurrence, and only then continue.";
    const structured = (response: Awaited<ReturnType<typeof mainMcpRequest>>): Record<string, unknown> | null => {
    ...
    const sessionMap = await mainMcpRequest(origin, token, 4, "tools/call", { name: "getOperatorSessionMap", arguments: { governing_standards_ack: governingAck } }, sessionId);
    const sessionMapContent = structured(sessionMap);
    const sessionMapToken = typeof sessionMapContent?.session_map_token === "string" ? sessionMapContent.session_map_token : "";
    if (!sessionMapToken) return { ok: false, phase: "get_operator_session_map", status: sessionMap.status, content: sessionMapContent };
    const plannedAction = { capability: "verify_deployed_mcp_version", arguments: {} };
    const knowledge = await mainMcpRequest(origin, token, 5, "tools/call", { name: "getOperatorKnowledge", arguments: { session_map_token: sessionMapToken, planned_action: plannedAction, governing_standards_ack: governingAck } }, sessionId);
    const knowledgeContent = structured(knowledge);
    const knowledgeToken = typeof knowledgeContent?.knowledge_token === "string" ? knowledgeContent.knowledge_token : "";
    if (!knowledgeToken) return { ok: false, phase: "get_operator_knowledge", status: knowledge.status, content: knowledgeContent };
    ...
    const closure = await mainMcpRequest(origin, token, 8, "tools/call", { name: "closeOperatorAction", arguments: { action_execution_token: actionExecutionToken, verification: { verified: true, evidence: ["Recovery smoke completed the exact verify_deployed_mcp_version action through the five-stage lifecycle."], next_action: "Recovery smoke complete; preserve the verified Main runtime identity.", prevention_required: false }, governing_standards_ack: governingAck } }, sessionId);
    ...
    const requiredLifecycleTools = ["getOperatorSessionMap", "getOperatorKnowledge", "getOperatorLiveState", "executeOperatorReadAction", "executeOperatorEngineeringAction", "executeOperatorAction", "executeOperatorCaseAction", "executeOperatorHardeningAction", "closeOperatorAction"];
    const requiredLifecycleSequence = ["getOperatorSessionMap", "getOperatorKnowledge", "getOperatorLiveState", "executeOperatorReadAction|executeOperatorEngineeringAction|executeOperatorCaseAction|executeOperatorHardeningAction|executeOperatorAction", "closeOperatorAction"];
