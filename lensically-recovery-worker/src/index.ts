...
const TOOLS: Tool[] = [
  ...
  { name: "runGitHubWorkflow", title: "Run GitHub workflow", description: "Dispatch a typecheck, operator-tests, gpt-memory-tests, or exact-head main release. The public release action is compiled server-side to the verified worker deployment workflow.", inputSchema: { type: "object", properties: { task: { type: "string", enum: ["typecheck", "operator-tests", "gpt-memory-tests", "release"] }, ref: { type: "string" } }, required: ["task"], additionalProperties: false }, annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: true } },
...
async function toolCall(name: string, args: Record<string, unknown>, env: Env): Promise<Record<string, unknown>> {
  ...
  if (name === "runMainMcpSmoke") {
    ...
    const startup = await mainMcpRequest(origin, token, 4, "tools/call", { name: "executeLensicallyIntent", arguments: { objective: "Load the compact Lensically startup bootstrap.", intent: "startup", inputs: {} } }, sessionId);
    const accountKey = await mainMcpRequest(origin, token, 5, "tools/call", { name: "executeLensicallyIntent", arguments: { objective: "Select the Manifest Mental account key without loading account data.", intent: "initialize account handshake", inputs: { brand_key: "manifest" } } }, sessionId);
    const direct = await mainMcpRequest(origin, token, 6, "tools/call", { name: "getEngineeringAccessState", arguments: {} }, sessionId);
    const mapped = await mainMcpRequest(origin, token, 7, "tools/call", { name: "executeLensicallyIntent", arguments: { objective: "Read the current Lensically engineering access state.", intent: "get engineering access state", inputs: {} } }, sessionId);
    const scheduler = await mainMcpRequest(origin, token, 8, "tools/call", { name: "executeLensicallyIntent", arguments: { objective: "Read the current scheduled-post scheduler state.", intent: "get scheduled post scheduler state", inputs: {} } }, sessionId);
    const scheduledToday = await mainMcpRequest(origin, token, 9, "tools/call", { name: "executeLensicallyIntent", arguments: { objective: "List Manifest Mental scheduled posts for the requested date.", intent: "list scheduled posts", inputs: { brand_key: "manifest_mental", date: "2026-07-18", timezone: "America/New_York", proceed_confirmed: true } } }, sessionId);
    const scheduledTomorrow = await mainMcpRequest(origin, token, 10, "tools/call", { name: "executeLensicallyIntent", arguments: { objective: "List Manifest Mental scheduled posts for the requested date.", intent: "list scheduled posts", inputs: { brand_key: "manifest_mental", date: "2026-07-19", timezone: "America/New_York", proceed_confirmed: true } } }, sessionId);
