...
const TOOLS: Tool[] = [
  ...
  { name: "restoreSchedulerNormal", title: "Restore scheduler normal", description: "Restore automatic scheduled-post publishing when there are zero overdue rows. Requires the owner's exact current-conversation authorization.", inputSchema: { type: "object", properties: { reason: { type: "string" }, owner_response: { type: "string" } }, required: ["reason", "owner_response"], additionalProperties: false }, annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: true } },
...
async function toolCall(name: string, args: Record<string, unknown>, env: Env): Promise<Record<string, unknown>> {
  ...
  if (name === "runMainMcpSmoke") {
    ...
    const schedulerRestoreContent = schedulerRestore ? (((schedulerRestore.body?.result as Record<string, unknown> | undefined)?.structuredContent as Record<string, unknown> | undefined) ?? null) : null;
