...
async function toolCall(name: string, args: Record<string, unknown>, env: Env): Promise<Record<string, unknown>> {
  ...
  if (name === "runMainMcpSmoke") {
    ...
    const campaign = await mainMcpRequest(origin, token, 9, "tools/call", { name: "executeLensicallyIntent", arguments: { objective: "Review one.", intent: "checkup", inputs: {} } }, sessionId);
