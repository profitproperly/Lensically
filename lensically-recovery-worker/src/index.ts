...
async function toolCall(name: string, args: Record<string, unknown>, env: Env): Promise<Record<string, unknown>> {
  ...
  if (name === "runMainMcpSmoke") {
    ...
    return {
      ...
      legacy_freehand_compatibility: { status: legacyMapped.status, ok: legacyCompatibilitySucceeded, profile_id: (legacyMappedContent?.routed_execution as Record<string, unknown> | undefined)?.profile_id ?? null, executed_tool: (legacyMappedContent?.routed_execution as Record<string, unknown> | undefined)?.executed_tool ?? null },
