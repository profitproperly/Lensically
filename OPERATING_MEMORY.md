...
- For ChatGPT GPT editor Configure scrolling, do not waste attempts on page-level `tab.dom_cua.scroll()` or wheel events at stale coordinates. First screenshot to identify the left Configure pane and its scrollbar. If wheel scroll does not move the panel once, drag the visible pane scrollbar/thumb directly, then inspect `dom_cua` for Actions. In the 1536x650 Chrome viewport, the working pattern was dragging the left pane scrollbar near `x≈752` down from `y≈150` toward `y≈520`.
