...
- Fresh ChatGPT Operator sessions use the normalized five-stage lifecycle. Step 0 MCP initialize is a tiny three-line bootloader; Step 1 `getOperatorSessionMap` holds only the recursive skeleton/pointers; Step 2 `getOperatorKnowledge` receives one typed planned action and mechanically derives its durable knowledge from the deployment-local machine-readable knowledge registry with no runtime repository fetch; Step 3 `getOperatorLiveState` mechanically derives that same action's mutable truth and account target; Step 4 `executeOperatorAction` may execute only the unchanged prepared action; Step 5 `closeOperatorAction` owns verification/checkpoint closure. Initial boot is 1-5 and later meaningful work re-enters at 2-5.
...
- White-label parity is maintained by source-owned automation, not by memory or chat promises. Use `npm run parity:white-label -- --apply --push` or `.github/workflows/lensically-white-label-parity.yml` to clone `profitproperly/Lensically-Operator-Threads`, classify deltas, copy only product-safe allowlisted files, validate the product repository, then advance `.lensically-parity/state.json`. Never mirror private Lensically continuation/state, production resources, seller-only commercial pages, database contents, credentials, or account data into the product repo. Product-relevant but unsafe or unmapped deltas must remain `PENDING` until a product-native transplant rule exists.
- M-BRAIN parity is maintained by the source-owned outbox generator, not by ad hoc chat summaries. Use `npm run mbrain:delta` after verified Lensically or product parity state changes to generate `.mbrain-delta/latest.json`, then apply the generated canonical payloads through M-BRAIN Gateway `writeDurableDelta` with the current `expected_version`. The writer must reject stale phrases and M-BRAIN must follow fresher `ENGINEERING_CONTINUATION.md`, runtime, GitHub main, and product parity state.
...
- Failed: running `git pull` inside a clean temporary Lensically worktree created from `origin/main` can fail because the worktree is detached. Use: attach it before update/work with `git checkout -B <work-branch> origin/main`, then commit and push explicitly with `git push origin HEAD:main`. Applies when: using disposable worktrees such as `C:\Auto-Threads\lensically-parity-worktree` for clean repo parity or repair work.
