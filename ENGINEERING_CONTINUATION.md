...
- Current interrupt action: make `beginManifestShadowRun` inspect any active different-operation run before blocking; if no durable snapshot runtime state exists, fail and archive that abandoned run immediately, then start the fresh operation. Preserve fail-closed blocking when durable state exists. Add both abandoned-run retirement and live-state noninterference regressions, release exact SHA, run a fresh `normal_24` preparation to a complete decision bundle, then close this interrupt and resume all three acceptance stages.
