...
- Current interrupt action: replace per-character encoding with one bulk UTF-8 encode and deterministic code-point-safe byte-boundary slicing; enforce a regression that the chunker performs one encode call while preserving exact Unicode round-trip and per-chunk byte limits. Release exact SHA, start a fresh operation that automatically retires `shadow-281060...`, verify complete live preparation, then close this interrupt and resume all three acceptance stages.
