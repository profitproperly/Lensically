...
current_action: Require every public lifecycle stage to honor durable release authority even when classified as direct engineering, while preserving the stale-release exemption only for non-public internal engineering repair actions; add regression/preflight prevention, validate and release one exact head, then re-run the exact repository_file_read Steps 1→5 path and deployed-version verifier before terminal ledger closure.
