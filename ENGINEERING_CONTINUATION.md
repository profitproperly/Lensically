...
- Current interrupt action: make exact-operation replay rebuild any incomplete preparation that has no durable runtime state, refresh its run lease/status in place, add a regression for interruption after run creation but before snapshot state, release the exact tested SHA, replay `main-mimic-parity-live24-2026-07-31-v3` to a complete decision bundle, then close this interrupt and resume all three acceptance stages without redoing completed parity architecture.
