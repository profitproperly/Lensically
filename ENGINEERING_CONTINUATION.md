...
current_action: Generalize the mandatory MCP refresh boundary to public MCP runtime-behavior releases, make stale public-lifecycle release-authority responses require client refresh rather than retry, add preflight prevention, validate and release one final exact head, then perform the mandated MCP refresh/context-port handoff. In the refreshed chat, run the exact repository_file_read Steps 1→5 replay and deployed-version verifier, then terminally close the ledger only after both succeed.
