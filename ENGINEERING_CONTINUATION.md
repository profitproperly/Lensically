...
current_action: Validate the source-controlled opaque-token rule, proven pre-dispatch client-block rule, normalized Recovery Main-lifecycle smoke, Recovery post-deploy live-smoke gate, and exact-SHA validation dispatch repair on one exact final head. Release Main and Recovery from that exact head, live-verify Recovery independently, then perform the mandatory fresh Main MCP connection handoff because Main runtime governance changed. In the refreshed connection, replay `verify_deployed_mcp_version` Steps 2→5 exactly once with byte-for-byte tokens, close incident `cbc1e9c0-1d25-4a7e-b3de-6eecdeb374c9` only after verified prevention, then terminally close and read back this ledger through the normal Main lifecycle.
