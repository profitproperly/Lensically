...
live_verification_findings: Refreshed production correctly advertises exactly five lifecycle tools on MCP 1.45.0 and commit c589288e9e44d80b97f7340f7c859b328ed13918, but `verifyDeployedMcpVersion` still requires the retired public `persist_manifest_autonomous_post` schema and therefore falsely returns `ok:false`; lifecycle tokens that begin with no MCP session binding also reject a later first valid `mcp-session-id` header as `operator_lifecycle_session_changed`; expected lifecycle guard rejections are missing from the hardening expected-control taxonomy and can create false P1 incidents.
current_action: Repair only the live verification defects without redesigning the normalized lifecycle: migrate `verifyDeployedMcpVersion` to the exact five-tool public contract, make an unbound lifecycle token adopt the first valid MCP session id while preserving mismatch rejection after binding, classify expected lifecycle guard rejections as control flow, add focused regressions for all three behaviors, validate one exact head, release that exact SHA, live-verify a harmless Steps 1-5 action, close the hardening interruption, then close this job.
