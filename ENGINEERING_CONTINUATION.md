...
active_checkpoint: validate-main-native-recovery
ecl_active_count: 1
...
current_action: Validate the exact Main-native recovery change set, run the required Main regressions, release the exact tested SHA, live-verify the upgraded lifecycle and recovery boundary, parity-close Main, then retire the separate Recovery fleet member and runtime only after successful proof.
...
global_mandate_update: M-BRAIN durable delta wiring is now the default future-project obligation for active Codex work. At every project startup and before meaningful closure, the model must use an existing writer or create the smallest correct project writer/node pair, then apply version-checked Gateway deltas without waiting for owner prompting.
current_action: None. Lensically-to-M-BRAIN durable delta writing is wired and validated. Future project adoption is mandatory during active Codex project sessions; this is not a claim that a background daemon acts while no project session is running.
