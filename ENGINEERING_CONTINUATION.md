...
- Current interrupt action: persist compact verified references to the canonical frozen-seed evidence instead of duplicate full payloads, hydrate exact candidates/evidence on every runtime-state read with snapshot-hash verification, add regression proving compact persistence and lossless hydration through strategy/batch execution, release exact SHA, verify complete live 24-slot preparation, then close this interrupt and resume all three acceptance stages.
