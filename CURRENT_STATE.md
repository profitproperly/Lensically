...
- `operator-execution-policy-v2` is now subordinate to the Mandatory Execution Map. `executeMappedIntent` resolves the action before any internal tool can be selected, then applies typed-schema normalization, the lower-level pre-call phonebook, continuity, authorization, and idempotency. Direct operational calls and direct guard calls are rejected before tool lookup. Known procedures are compulsory. Unknown or stale terrain is the only condition that can issue a signed discovery permit, and an open discovery incident keeps the interrupted objective paused until a successful path is promoted and activated.
