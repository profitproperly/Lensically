...
- MCP v1.14.1 hardens the ChatGPT client boundary. `confirmOperatorProceed` persists continuity server-side and returns the exact client-safe next-call key: `manifestmental`, `opmgdeadman`, or `vectrix`. Later account calls require only their direct typed arguments plus `proceed_confirmed=true`; `continuity_loaded` is no longer advertised or accepted as a caller contract. Generic `listMcpTools.arguments` account/runtime bridges are blocked and reserved for source-defined engineering/admin fallbacks. Protected decisions use a five-field compact proposal with bounded server defaults, and OpsMemory payloads are schema-bounded.
