...
- Validation plane priority is Local Execution Node first, Cloudflare Workers Builds second, and GitHub Actions third. The local node is a Windows outbound-polling Scheduled Task host with no inbound port; it queues and executes signed exact-SHA jobs, reports `local-validation-receipt-v1`, and supports remotely replaceable worker slots with automatic rollback.
- Local node enrollment uses short-lived single-use tokens and per-node credentials. Main stores only credential hashes, can report enrollment/heartbeat/revocation state, and can revoke one node without rotating any global Cloudflare secret.
...
- Cloudflare Workers Builds is connected to `profitproperly/Lensically` on `main` with root `lensically-worker`. Ordinary commits run `npm ci && npm run validate:cloudflare`; `npm run deploy:cloudflare-gated` skips deployment unless the same validated commit carries `[verified-worker-release]` and its receipt SHA matches. The deploy gate accepts either `cloudflare-validation-receipt-v1` or `local-validation-receipt-v1` when every exact-SHA field matches the release candidate.
