...
- Operator MCP v1.26.0 replaces the separate map, phonebook, and passive-memory behavior with one mandatory execution library consulted before every routed action. It compiles repository operating knowledge, Ops Memory, pre-call routes, workflow requirements, admin failures, engineering audits, execution events, operational incidents, map entries/incidents/attempts/promotions, open backlog, account strategy memory, the D1 table catalog, and the typed tool registry. The library records failures before repair, permits discovery only for unknown terrain or a verified path that actually failed, and records the promoted replacement before the interrupted objective resumes.
