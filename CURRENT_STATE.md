...
- MCP v1.17.0 hardens the ChatGPT client boundary, persists unresolved scheduled-delivery incidents across fresh chats, and atomically converts an explicit owner approval into durable authorization before the first protected scheduler, destructive, rollback, or tool-control call. `getOperatorStartupContext` is the only startup exemption; every later Lensically action must first call the directly advertised `guardLensicallyCall` with its exact JSON arguments. The guard normalizes bounded schema values, blocks known client-preflight and route failures, and returns a signed exact-payload `execution_guard` required by the dispatcher. `confirmOperatorProceed` still persists continuity server-side and client-safe aliases remain `manifestmental`, `opmgdeadman`, and `vectrix`.
