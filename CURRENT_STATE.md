...
- Every MCP initialize response, deliberate startup bootstrap, and engineering precheck begins with `operator-governing-standards-v3`: **Autonomy, Efficiency, Prevention**. It is the highest Lensically operating authority, requires the fastest complete route without rushing, skipping, bypassing controls, bypassing fixes, or using workarounds instead of root repairs, and requires every failure to become a recorded permanent prevention before work resumes.
