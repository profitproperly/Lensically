...
- MCP v1.21.0 adds `operator-pre-call-routing-v1`, a universal DB-backed successful-path phonebook consulted by `guardLensicallyCall` before token issuance and rechecked by the dispatcher immediately before execution. Verified routes may rewrite arguments or require a different tool before the original tool can run; those routes override model judgment and survive fresh chats. `recordPreCallRoute` persists a newly verified path with matching conditions, OpsMemory linkage, and engineering audit evidence, while `listPreCallRoutes` exposes persistent and source-defined routes. The existing continuity, protected-approval, scheduler-canary, compact client-boundary, and safe account-alias behavior remains active.
