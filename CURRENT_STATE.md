...
- Owner rejection enforcement is universal with account-scoped evidence. Every generation run loads and persists the selected account's rejected drafts and rejection-feedback memory. `historical_owner_rejection_gate` blocks explicit banned surfaces, close rejected-draft matches, partial or missing model review, and mismatched rejection-context fingerprints. `required_gate_execution_gate` blocks `showable=true` whenever any active blocking gate did not execute with an auditable result. The startup bootstrap and MCP initialize instructions require this behavior in fresh chats.
