...
- Validation plane priority is Local Execution Node first, Cloudflare Workers Builds second, and GitHub Actions third. The local node is a Windows outbound-polling service with no inbound port; it queues and executes signed exact-SHA jobs, reports `local-validation-receipt-v1`, and supports remotely replaceable worker slots with automatic rollback.
- One-time local node installation is `powershell -ExecutionPolicy Bypass -File .\lensically-local-node\scripts\install-local-node.ps1 -NodeId brian-win-node -LensicallyOrigin https://api.lensically.com`.
