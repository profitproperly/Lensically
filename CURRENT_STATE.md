...
- Operator MCP v1.9.0 uses handle-free automatic continuity, calendar-first production, scheduler canary isolation, owner-ratified account governance, persistent recursive engineering authority, and autonomous six-hour performance collection. Fresh ChatGPT sessions load `getOperatorStartupContext`, including `operator-engineering-authority-v1` and `operator-execution-policy-v2`, before any engineering call. The key handshake and `confirmOperatorProceed` still restore canonical account schedule, workflow, source claims, review batches, and pending account decisions. Routine repository work, tests, workflow repair, deployment, runtime verification, routing, MCP infrastructure, and universal engineering improvements execute without owner proposals or numerical tool budgets. Account content, scheduling, strategy, destructive data actions, ownership changes, and irreversible business decisions remain separately protected.
