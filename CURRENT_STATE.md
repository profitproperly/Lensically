...
- Operator MCP v1.4.1 is continuity hardened with platform-safe references. Fresh ChatGPT sessions load `getOperatorStartupContext`, perform the exact key handshake, receive an opaque `continuation_ref` UUID from `confirmOperatorProceed`, ask resume or start fresh, then call `resolveContinuationContext`. The backend returns one canonical database-backed capsule containing the workflow checkpoint, last completed action, next pending owner checkpoint, active artifact IDs, source-batch progress, execution policy, stable operation identity, repository/runtime identity, and a second opaque `continuity_ref`. Every later account-scoped call requires `proceed_confirmed=true` and that reference; conversation memory cannot substitute for workflow state. Resume forbids redraws, restarts, and duplicate work. Start fresh preserves previous sessions and artifacts.
