...
- GPT growth and generation contexts link posted strategy tags and source/generation lineage to the post's own metric history. Post learning is normalized at 6, 12, 24, 48, and 72-hour maturity checkpoints; account follower totals remain separate trajectory data and are never attributed to a post, posting day, batch, or posting period.
...
- Operator MCP v1.11.0 adds `performance-evaluator-v1` to handle-free continuity, calendar-first production, scheduler safety, recursive engineering authority, six-hour collection, and the optimized engineering pipeline. The evaluator persists content fingerprints, selects comparable 6/12/24/48/72-hour snapshots, scores reach/resonance/propagation/conversation against same-maturity cohorts, aggregates feature evidence, creates confidence-scored hypotheses, tracks fatigue, and produces generation/source-selection briefs with adaptive exploitation, improvement, and exploration allocations. `get_performance_learning` exposes the current brief. Follower count is account-level goal trajectory only; post/day/batch/period attribution is forbidden. Related repository edits use atomic patch sets and normal releases use one exact-SHA workflow.
...
- `operator-autonomy-governance-v2` separates account governance from engineering authority. Manifest content, scheduling, strategy, destructive data operations, ownership changes, and irreversible business decisions remain owner-ratified. `operator-engineering-authority-v1` gives the model persistent full discretion for routine Lensically engineering: investigate, patch, test, retry with changed inputs, deploy, verify, repair, and improve without owner proposals or numerical tool budgets. Engineering friction is recorded in telemetry and audit history; owner-facing chat surfaces completed outcomes, meaningful risk, or a true owner-required blocker.
...
- Manifest owner interaction is AI-led and calendar-aware. After continuity loads, Lensically restores pending decisions, inspects schedule coverage and available evidence, and selects the next operational priority rather than asking the owner to supply routine direction. The model persists and presents its decision before mutation; the owner ratifies, rejects, or revises it during the training phase. For an approved content decision, source selection, source-card creation/reuse, locking, generation, self-rejection, submission, and gate evaluation run silently. The owner receives four numbered items at a time, each showing only the original Source and Generated post. Backend IDs and provisional posting times stay hidden. Approved posts schedule into the earliest open hourly slots in item order, and confirmed times are reported only after persistence succeeds. Stored draft strategy metadata is normalized before batch strategy-tag persistence so sparse or older drafts cannot interrupt scheduling after only part of a review batch is saved.
