...
- Operator MCP v1.22.0 keeps `operator-pre-call-routing-v1` as the universal DB-backed successful-path phonebook and adds `operator-routed-execution-gateway-v1`. Public MCP discovery exposes only startup plus `routeAndExecuteLensicallyCall`; every operational intent is resolved, rewritten or redirected, schema-validated, and signed inside the gateway before the underlying tool handler can be selected. Direct operational tool names are rejected even when supplied with an older valid guard token. Verified routes may rewrite arguments or require a different tool before the original tool can run; those routes override model judgment and survive fresh chats. `recordPreCallRoute` persists a newly verified path with matching conditions, OpsMemory linkage, and engineering audit evidence, while `listPreCallRoutes` exposes persistent and source-defined routes. The existing continuity, protected-approval, scheduler-canary, compact client-boundary, and safe account-alias behavior remains active.
...
- `operator-execution-policy-v2` is now enforced through `routeAndExecuteLensicallyCall`. The gateway reads persistent and source-defined routes, validates the intended tool schema, normalizes safe bounds, follows verified redirects internally, and signs only the final routed payload. The dispatcher rejects every direct operational call before tool lookup, continuity, idempotency, handlers, or external requests run. Newly solved reusable blockers must be added to this guard path, regression coverage, engineering audit, and operating memory before the original objective resumes.
