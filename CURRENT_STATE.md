...
- Operator MCP v1.4.3 is continuity hardened with handle-free server state and a persisted source-progress anchor. Fresh ChatGPT sessions load `getOperatorStartupContext`, perform the exact key handshake, call `confirmOperatorProceed` to create a short-lived D1 confirmation, ask resume or start fresh, then call `resolveContinuationContext` with only the explicit owner choice. The backend returns one canonical database-backed capsule containing the workflow checkpoint, last completed action, next pending owner checkpoint, active artifact IDs, source-batch progress, execution policy, stable operation identity, and repository/runtime identity. Source selections persist `pending`, `linked`, and `skipped` dispositions plus their owner-facing workflow sequence. Resume anchors after the highest draw linked to the latest processed workflow sequence, preserves earlier unlinked draws as historical skips, and advances to the next pending draw without rewinding. Every later account-scoped call requires `proceed_confirmed=true` and `continuity_loaded=true`; the backend verifies the latest D1 continuity state for that brand. Conversation memory cannot substitute for workflow state, and no generated token, nonce, UUID, or reference is passed between tools. Resume forbids redraws, restarts, and duplicate work. Start fresh preserves previous sessions and artifacts.
...
- Owner interaction is universally fixed: after a source card is approved and locked, generation-run creation, candidate generation, self-rejection, submission, and gate evaluation run silently. The next owner decision occurs only when a passing draft has `showable=true` and is presented for approve/reject/revise; scheduling confirmation remains separate after draft approval.
- Manifest generation uses close source mimicry: preserve the original hook, structure, meaning, tone, and payoff; make only slight wording changes; invent no new scene or premise; block exact source copies. Manifest generation runs persist a compact explicit-hard-ban context instead of full rejected-draft payloads. `historical_owner_rejection_gate` blocks only owner-explicit hard bans for Manifest and requires no model fingerprint review. OPMG and Vectrix retain their configured rejection behavior. `required_gate_execution_gate` still blocks `showable=true` whenever any active blocking gate did not execute with an auditable result. The startup bootstrap and MCP initialize instructions carry this behavior into fresh chats.
