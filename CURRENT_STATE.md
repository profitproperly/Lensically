...
- MCP v1.20.0 hardens the ChatGPT client boundary, persists unresolved scheduled-delivery incidents across fresh chats, atomically converts explicit owner approval into durable authorization, exposes compact scheduler controls directly, and automatically consumes the next approved one-post canary decision at the scheduler trigger. The canary no longer depends on the originating chat retaining or refreshing a tool schema. `getOperatorStartupContext` is the only startup exemption; every later Lensically action must first call the directly advertised `guardLensicallyCall` with its exact JSON arguments. The guard normalizes bounded schema values, blocks known client-preflight and route failures, and returns a signed exact-payload `execution_guard` required by the dispatcher. `confirmOperatorProceed` still persists continuity server-side and client-safe aliases remain `manifestmental`, `opmgdeadman`, and `vectrix`.
