...
- Fresh model sessions assume zero retained Lensically knowledge. `getOperatorStartupContext` now returns `operator-session-competency-v1`: a mandatory structured shop map covering canonical sources, repository/tool boundaries, MCP/schema operation, GitHub/Cloudflare release paths, web-surface ownership, continuation/state authority, failure/prevention behavior, payload/idempotency safety, and the active capability index. The model pays this startup tax before non-startup work, then loads only task-specific live state.
