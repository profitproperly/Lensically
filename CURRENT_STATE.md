...
- Operator MCP v1.10.0 uses handle-free automatic continuity, calendar-first production, scheduler canary isolation, owner-ratified account governance, persistent recursive engineering authority, autonomous six-hour performance collection, and the optimized engineering pipeline. Fresh ChatGPT sessions load `getOperatorStartupContext`, including `operator-engineering-authority-v1` and `operator-execution-policy-v2`, before any engineering call. Related repository edits must use one atomic patch set; normal releases must use one exact-SHA workflow rather than separate typecheck, test, and deploy runs. Account content, scheduling, strategy, destructive data actions, ownership changes, and irreversible business decisions remain separately protected.
