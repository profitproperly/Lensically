...
- The public request contains a registered `profile_id` and bounded `inputs`. Freehand `objective` and `intent` fields and direct internal tool calls are rejected; canonical semantic requests compile server-side before routing.
