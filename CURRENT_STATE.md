...
- The permanent public request contract is concise `objective`, `intent`, and bounded `inputs`, with optional continuation fields. Account initialization remains first-class through exact `startup`, key-selection, and Proceed intent shapes. Direct internal tool calls remain rejected.
