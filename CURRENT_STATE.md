...
- Operator MCP v1.4.2 is continuity hardened with handle-free server state. Fresh ChatGPT sessions load `getOperatorStartupContext`, perform the exact key handshake, call `confirmOperatorProceed` to create a short-lived D1 confirmation, ask resume or start fresh, then call `resolveContinuationContext` with only the explicit owner choice. The backend returns one canonical database-backed capsule containing the workflow checkpoint, last completed action, next pending owner checkpoint, active artifact IDs, source-batch progress, execution policy, stable operation identity, and repository/runtime identity. Every later account-scoped call requires `proceed_confirmed=true` and `continuity_loaded=true`; the backend verifies the latest D1 continuity state for that brand. Conversation memory cannot substitute for workflow state, and no generated token, nonce, UUID, or reference is passed between tools. Resume forbids redraws, restarts, and duplicate work. Start fresh preserves previous sessions and artifacts.
