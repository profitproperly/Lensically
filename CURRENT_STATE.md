...
- Operator MCP v1.4.0 is continuity hardened. Fresh ChatGPT sessions load `getOperatorStartupContext`, perform the exact key handshake, receive a signed nonce from `confirmOperatorProceed`, ask resume or start fresh, then call `resolveContinuationContext`. The backend returns one canonical database-backed capsule containing the workflow checkpoint, last completed action, next pending owner checkpoint, active artifact IDs, source-batch progress, execution policy, stable operation identity, repository/runtime identity, and a signed continuity token. Every later account-scoped call requires `proceed_confirmed=true` and that token; conversation memory cannot substitute for workflow state. Resume forbids redraws, restarts, and duplicate work. Start fresh preserves previous sessions and artifacts.
