...
- Operator MCP v1.19.0 uses `performance-evaluator-v2` with the existing newest-40 full-Insights collection unchanged. The evaluator persists content fingerprints, selects comparable 6/12/18/24-hour snapshots, treats 24 hours as final, invalidates obsolete 48/72-hour learning rows, scores reach/resonance/propagation/conversation against same-maturity cohorts, aggregates feature evidence, creates confidence-scored hypotheses, tracks fatigue, and produces generation/source-selection briefs with adaptive exploitation, improvement, and exploration allocations. It does not make targeted calls for older posts. `get_performance_learning` exposes only current-version evidence. Follower count is account-level goal trajectory only; post/day/batch/period attribution is forbidden.
- MCP v1.19.0 hardens the ChatGPT client boundary, persists unresolved scheduled-delivery incidents across fresh chats, atomically converts explicit owner approval into durable authorization before the first protected call, prioritizes scheduler controls inside constrained direct tool surfaces, and adds `runApprovedPostCanary` as a compact protected command that runs exactly one post and returns the scheduler to paused without a client-sensitive mode payload. `getOperatorStartupContext` is the only startup exemption; every later Lensically action must first call the directly advertised `guardLensicallyCall` with its exact JSON arguments. The guard normalizes bounded schema values, blocks known client-preflight and route failures, and returns a signed exact-payload `execution_guard` required by the dispatcher. `confirmOperatorProceed` still persists continuity server-side and client-safe aliases remain `manifestmental`, `opmgdeadman`, and `vectrix`.
...
- `operator-execution-policy-v2` is now enforced through `guardLensicallyCall`. The guard reads the source-defined known path, validates the intended tool schema, normalizes safe numeric bounds, blocks diagnosed client-sensitive or known-bad routes, and signs the exact payload. The dispatcher refuses every non-exempt call without a matching token before continuity, idempotency, handlers, or external requests run. Newly solved reusable blockers must be added to this guard path, regression coverage, engineering audit, and operating memory before the original objective resumes.
