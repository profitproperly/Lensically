...
- Operator Mode also exposes a real JSON-RPC MCP endpoint at `/api/operator/mcp` for ChatGPT App/Connector use. It advertises 24 operator tools, uses the same bearer/internal auth as the operator tool routes, and returns MCP `structuredContent` around the existing backend tool behavior.
