...
- Validation plane priority is Local Execution Node first, Cloudflare Workers Builds second, and GitHub Actions third. The local node is a Windows outbound-polling Scheduled Task host with no inbound port; it queues and executes signed exact-SHA jobs in node-controlled isolated worktrees, returns raw `local-stage-evidence-v1`, and supports remotely replaceable worker slots with automatic rollback.
...
- Cloudflare Workers Builds is connected to `profitproperly/Lensically` on `main` with root `lensically-worker`. Ordinary commits run `npm ci && npm run validate:cloudflare`; `npm run deploy:cloudflare-gated` skips deployment unless the same validated commit carries `[verified-worker-release]` and its Cloudflare exact-head receipt SHA matches. Copied local receipt files never authorize production deployment; local validation receipts are server-issued durable records used by the controlled local deploy job path.
