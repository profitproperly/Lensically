...
- The public request contains a registered `profile_id` and bounded `inputs`. Account initialization is enrolled in the same registry through first-class `startup`, `account_key_selection`, and `account_proceed` profiles. Exact fresh-account shapes with a nested or omitted ID are normalized into one of those registered lifecycle profiles before routing; arbitrary freehand requests and direct internal tool calls remain rejected.
