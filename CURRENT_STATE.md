...
- Fresh model sessions assume zero retained Lensically knowledge. `getOperatorStartupContext` now returns `operator-session-competency-v2`, a **constant-size competency router** rather than a growing shop dump. It points to fixed broad domains in root `OPERATOR_COMPETENCY.md`; the model loads only the bounded relevant domain plus task-specific live state. New tools, procedures, providers, hazards, and product details update the handbook domain and do not grow startup. The 6,500-character initialize ceiling and 24,000-byte transport ceiling are guardrails, not storage targets, and may not be raised or managed by deleting unrelated competence.


- Every MCP initialize response and deliberate startup bootstrap begins with `operator-governing-standards-v7`: **Autonomy, Efficiency, Prevention**. It is the highest Lensically operating authority, requires the fastest complete route without rushing, skipping, bypassing controls, bypassing fixes, or using workarounds instead of root repairs, and requires every failure to become a recorded permanent prevention before work resumes. Routine `engineeringPrecheck` returns a compact authority reference after the one-per-session competency boot instead of repeating the full startup payload.
