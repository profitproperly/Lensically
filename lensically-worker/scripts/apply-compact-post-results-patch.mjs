...
const temporaryStep = `\n      - name: Apply one-run compact post-results maintenance\n        if: \${{ inputs.task == 'typecheck' }}\n        working-directory: .\n        run: |\n          set -o pipefail\n          if [ -f lensically-worker/scripts/apply-compact-post-results-patch.mjs ]; then\n            if ! node lensically-worker/scripts/apply-compact-post-results-patch.mjs 2>&1 | tee /tmp/compact-post-results-maintenance.log; then\n              diagnostic=$(tail -n 80 /tmp/compact-post-results-maintenance.log | tr '\\n' ' ' | sed 's/%/%25/g')\n              echo "::error title=Compact post-results maintenance failure::$diagnostic"\n              printf '%s\\n' "$diagnostic" > lensically-worker/compact-maintenance-error.txt\n              git config user.name "lensically-engineering"\n              git config user.email "lensically-engineering@users.noreply.github.com"\n              git add lensically-worker/compact-maintenance-error.txt\n              git commit -m "Record compact maintenance diagnostic"\n              git push origin HEAD:main\n              exit 1\n            fi\n            git config user.name "lensically-engineering"\n            git config user.email "lensically-engineering@users.noreply.github.com"\n            git add -A\n            git commit -m "Add compact published-post verification [operator-tests]"\n            git push origin HEAD:main\n          fi\n`;
