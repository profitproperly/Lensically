...
const steps = [
  ...
  ["npm", ["run", "test", "--", "--run", "test/localExecutionIntegration.spec.ts", "--reporter=dot", "--bail=1"], "Local execution integration"],
  ["node", ["scripts/test-local-worker-receipt.mjs"], "Local worker receipt"],
  ["node", ["scripts/test-local-worker-update-rollback.mjs"], "Local worker update and rollback"],
  ["powershell", ["-ExecutionPolicy", "Bypass", "-File", "../lensically-local-node/scripts/test-local-node-commissioning.ps1"], "Local node commissioning"],
