...
const escapeRegex = (value) => value.replace(/[.*+?^${}()|[\]\\]/g, "\\const uniqueActiveTitles = Array.from(new Set(activeDefinitions.map((test) => test.title))).sort((left, right) => left.localeCompare(right));
...
const titlePattern = `(?:${selectedTitles.map(escapeRegex).join("|")})import { spawnSync } from "node:child_process";
import { readFileSync } from "node:fs";
import { resolve } from "node:path";
