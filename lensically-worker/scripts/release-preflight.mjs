...
if (!executionMap.includes('EXECUTION_POLICY_LIBRARY_VERSION = "execution-policy-library-v2"')
    ...
    || !source.includes("prepareSourceDefinedDirectEngineeringCall")
    || !source.includes('mode === "source_defined_direct_engineering"')
    || !source.includes("d1_bootstrap_bypassed: true")
    ...
    || !releasePreflightSource.includes("executionMap.replace(generatedPattern, () => generatedBlock)")
    || !releasePreflightSource.includes('"lensically-worker/scripts/release-preflight.mjs"')
    || !releasePreflightSource.includes("const generatedPattern = /^")
    ...
    || !generatedKnowledgeIsJavaScriptSafe
