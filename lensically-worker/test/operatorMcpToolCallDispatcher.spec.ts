...
describe("Operator MCP tool-call dispatcher", () => {
  ...
  it("preserves direct-public admission and rejects hidden routes", async () => {
    ...
    const response = await dispatchOperatorMcpToolCall({
      request: new Request("https://lensically.test/mcp", { method: "POST" }),
      id: 21,
            params: { name: "executeOperatorReadAction", arguments: { live_state_token: "live-token", execution_descriptor: { action_id: "unknown_profile", effect_class: "read_only" } } },
    }, dependencies);
    expect(await structuredContent(response)).toMatchObject({
      ok: false,
      error: "unknown_profile",
    ...
    const response = await dispatchOperatorMcpToolCall({
      request: new Request("https://lensically.test/mcp", { method: "POST" }),
      id: 3,
            params: { name: "executeOperatorReadAction", arguments: { live_state_token: "live-token", execution_descriptor: { action_id: "repository_status", effect_class: "read_only" } } },
    }, dependencies);
    expect(await structuredContent(response)).toMatchObject({
      ok: false,
      error: "pre_call_route_required",
