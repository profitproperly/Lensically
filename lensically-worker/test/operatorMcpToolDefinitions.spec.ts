...
import { OPERATOR_DISCOVERY_EXECUTION_RULE, OPERATOR_FAILURE_REPAIR_RULE, OPERATOR_GOVERNING_STANDARDS_ACK } from "../src/operatorMcpProtocol";
  ...
  it("builds three scoped account wrappers and preserves priority order", () => {
    ...
    for (const tool of tools) {
      ...
      const governingAck = (tool.inputSchema.properties as Record<string, Record<string, unknown>>).governing_standards_ack;
                  expect(String(governingAck.description)).toContain(OPERATOR_DISCOVERY_EXECUTION_RULE);
