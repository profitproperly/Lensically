...
describe("System Directory foundation", () => {
  ...
  it("rejects internal handler names and reserved routing keys before public calls", () => {
    ...
    expect(PREVENTED_CLIENT_BLOCKS.map((incident) => incident.id)).toEqual([
      "normalized_lifecycle_semantic_work_transition_branch",
      "normalized_lifecycle_semantic_hardening_branch",
  ...
  it("registers typed hardening and repository inspection profiles", () => {
    ...
    expect(resolveHardeningResumeResult({ resume_result: { source: "top_level" } }, { resume_result: { source: "nested" } }, { source: "stored" })).toEqual({ source: "top_level" });
    expect(resolveHardeningResumeResult({}, { resume_result: { source: "nested" } }, { source: "stored" })).toEqual({ source: "nested" });
  ...
  it("registers bounded work-state profiles and the complete hardening release sequence", () => {
    ...
    expect(CLIENT_SAFE_REQUEST_PROFILES.case_step).toMatchObject({ intent: "advance hardening incident", allowed_input_keys: ["stage", "case", "cause", "generalization", "rule", "tests", "ref", "deployment", "proof", "resume", "gain"] });
