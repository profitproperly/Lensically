...
describe("Operator MCP registry composition", () => {
  it("preserves the exact 55-tool account aggregation order", () => {
  ...
  it("builds the exact 112 direct tools with deterministic priority ordering", () => {
    ...
    expect(names.indexOf("get_performance_learning")).toBeLessThan(names.indexOf("prepare_manifest_autonomous_cycle"));
  ...
  it("builds all three scoped account wrapper surfaces without brand_key", () => {
    ...
    expect(scopedTools).toHaveLength(directTools.length + (54 * 3));
