  ...
  it("rejects an MCP session created by a previous Worker deployment before routing", async () => {
    ...
    for (const actionId of caseStepDescriptorIds) {
      expect(caseStepDescriptorBranches
        ...
        .map((branch) => branch.properties?.effect_class?.const)).toEqual(["read_only"]);
    }
    ...
    expect(descriptorActionIds.some((actionId) => actionId === "case_step" || actionId.startsWith("case_step_"))).toBe(false);
        expect(executionDescriptorBranches.length).toBe(plannedCapabilityEnums.length + 12);
    ...
    expect(caseActionTool?.description).not.toContain("case_step");
    expect(caseActionTool?.description).not.toContain("execution_descriptor");
    expect(caseActionTool?.description).toContain("opaque live-state token");
...
`);
  ...
  it("enforces the normalized Operator lifecycle from session map through explicit closure", async () => {
    ...
    expect(step5.next_sequence).toEqual(["getOperatorKnowledge", "getOperatorLiveState", "executeOperatorReadAction|executeOperatorCaseAction|executeOperatorHardeningAction|executeOperatorAction", "closeOperatorAction"]);
