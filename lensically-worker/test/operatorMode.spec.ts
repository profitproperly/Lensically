...
async function resetTables(): Promise<void> {
  ...
  delete (env as unknown as { LENSICALLY_MCP_OAUTH_CLIENT_SECRET?: string }).LENSICALLY_MCP_OAUTH_CLIENT_SECRET;
    ...
    "operator_workflow_sessions",
        ...
        await env.DB.prepare(`DROP TABLE IF EXISTS ${table}`).run();
                  ...
                  await env.DB.prepare("DELETE FROM operator_manifest_decision_influences").run();
  await env.DB.prepare("DELETE FROM operator_source_selection_plans").run();
  await env.DB.prepare("DELETE FROM operator_source_selection_receipts").run();
  await env.DB.prepare("DELETE FROM operator_source_family_label_transitions").run();
  await env.DB.prepare("DELETE FROM operator_source_family_evidence_states").run();
  ...
  await env.DB.prepare("DELETE FROM operator_manifest_learning_observations").run();
  await env.DB.prepare("DELETE FROM operator_manifest_comparable_analyses").run();
  await env.DB.prepare("DELETE FROM operator_manifest_maturity_evaluations").run();
  await env.DB.prepare("DELETE FROM operator_manifest_semantic_signatures").run();
  ...
  await env.DB.prepare("DELETE FROM operator_performance_hypotheses").run();
  ...
  await env.DB.prepare("DELETE FROM operator_work_state").run();
  ...
  await env.DB.prepare("DELETE FROM operator_autonomous_growth_cycles").run();
...
describe("operator mode MCP endpoint", () => {
  ...
  it("rejects an MCP session created by a previous Worker deployment before routing", async () => {
    ...
    expect(caseActionTool?.inputSchema?.properties).toHaveProperty("execution_descriptor");
    ...
    const caseMutationExecutionDescriptorSchema = caseActionTool?.inputSchema?.properties?.execution_descriptor as { oneOf?: Array<{ properties?: { action_id?: { const?: string }; effect_class?: { const?: string } } }> } | undefined;
    ...
    const caseMutationExecutionDescriptorBranches = caseMutationExecutionDescriptorSchema?.oneOf ?? [];
    ...
    expect(caseMutationExecutionDescriptorBranches.every((branch) => branch.properties?.effect_class?.const === "mutation")).toBe(true);
    expect(caseMutationExecutionDescriptorBranches).toHaveLength(11);
    ...
    const executionDescriptorBranches = [...mutationExecutionDescriptorBranches, ...caseMutationExecutionDescriptorBranches, ...readExecutionDescriptorBranches];
    ...
    for (const actionId of caseStepDescriptorIds) {
      expect(caseStepDescriptorBranches
        ...
        .map((branch) => branch.properties?.effect_class?.const).sort()).toEqual(["mutation", "read_only"]);
    }
    ...
    expect(descriptorActionIds.some((actionId) => actionId === "case_step" || actionId.startsWith("case_step_"))).toBe(false);
        expect(executionDescriptorBranches.length).toBe(plannedCapabilityEnums.length + 23);
    ...
    expect(caseActionTool?.inputSchema?.properties).not.toHaveProperty("profile_id");
    expect(caseActionTool?.inputSchema?.properties).not.toHaveProperty("inputs");
    expect(caseActionTool?.description).toContain("case_step");
    expect(caseActionTool?.description).toContain("execution_descriptor");
    ...
    expect(step1.lifecycle.initial_sequence).toEqual(["getOperatorSessionMap", "getOperatorKnowledge", "getOperatorLiveState", "executeOperatorReadAction|executeOperatorAction", "closeOperatorAction"]);
    expect(step1.lifecycle.recurring_sequence).toEqual(["getOperatorKnowledge", "getOperatorLiveState", "executeOperatorReadAction|executeOperatorAction", "closeOperatorAction"]);
    expect(step1.lifecycle.step4_gateways).toEqual({ read_only: "executeOperatorReadAction", mutation: "executeOperatorAction" });
    ...
    expect(repositorySearchKnowledge.action_rule_binding.action_intelligence_version).toBe("action-intelligence-binding-v1");
    expect(repositorySearchKnowledge.action_rule_binding.prevention_rule_ids).toEqual(expect.arrayContaining(["neutral_repository_symbol_search_contract", "client_safe_step4_execution_descriptor"]));
    expect(repositorySearchKnowledge.action_rule_binding.action_intelligence_ids).toEqual(expect.arrayContaining(["neutral_repository_symbol_search_contract", "client_safe_step4_execution_descriptor"]));
...
`);
  ...
  it("preserves client-visible read semantics for engineering continuation through Step 4", async () => {
    ...
    const executeCaseStep = async (
      ...
      expect(neutralCaseStepRule?.winning_path).toEqual(expect.arrayContaining([
  ...
  it("reconciles corrupted opaque lifecycle references by exact action binding when the client omits MCP session headers", async () => {
    ...
    const changedStep4 = await mcpToolCallRaw<{ ok: boolean; error?: string }>("executeOperatorReadAction", {
  ...
  it("keeps a sessionless lifecycle unbound across valid transport-session rotation while preserving strict bound-session mismatch rejection", async () => {
    ...
    expect(persistedLiveStatePayload.action_rule_registry_version).toBe(OPERATOR_ACTION_RULE_REGISTRY_VERSION);
    ...
    expect(persistedExecutionPayload.action_rule_registry_version).toBe(OPERATOR_ACTION_RULE_REGISTRY_VERSION);
  ...
  it("reconciles corrupted opaque lifecycle references only from the exact current MCP session and stage without replaying post-Step-3 actions", async () => {
    ...
    const step4 = await callWithSession<{ ok: boolean; action_execution_token: string }>(sessionId, "executeOperatorReadAction", {
  ...
  it.skip("reviews a scheduled autonomous post without making the owner an operational dependency", async () => {
    ...
    if (!persisted.success) {
      throw new Error(`scheduled_review_persist_failed:${JSON.stringify(persisted)}`);
    }
  ...
  it("treats exactly six public tools across the five normalized lifecycle stages as a healthy deployment boundary", () => {
  ...
  it("loads a compact source-defined session map", async () => {
    ...
    expect(sessionMap.lifecycle).toMatchObject({
      ...
      step4_gateways: { read_only: "executeOperatorReadAction", mutation: "executeOperatorAction" },
    });
  ...
  it("generalizes OpenAI client safety blocks across active and closed related incidents", async () => {
    ...
    const currentAliasCall = await mcpToolRaw<{
      ok: boolean;
      created: boolean;
      incident: { id: string; classification: string; severity: string };
      recurrence: { status: string; recurrence_family: string };
  ...
  it("routes bounded known-file repository search through the main Execution Kernel without Recovery", async () => {
    ...
    expect(campaign.isError, JSON.stringify(campaign.structuredContent)).not.toBe(true);
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                expect(campaign.structuredContent.campaign.passed).toBe(95);
  ...
  it.skip("retired: inspects unresolved delivery incidents before a guided Growth Mission discussion", async () => {
    ...
    expect(incident).toBeNull();
            ...
            it.skip("runs Manifest account mutations only after Guided Growth Mission approval and locks again on the next Proceed", async () => {
    await ensureMcpAccountOpen("manifest_mental");
  ...
  it.skip("retired: OpsMemory and engineering bridge compatibility", async () => {
    ...
    expect(missingReason.structuredContent).toMatchObject({
      ...
      validation_errors: expect.arrayContaining([
        { path: "$.planned_action.arguments.reason_code", error: "required_property_missing" },
      ]),
    });
  ...
  it.skip("retired: free-text bulk scheduled-post deletion and recovery history", async () => {
    ...
    await mcpTool("setScheduledPostSchedulerMode", {
      brand_key: BRAND_KEY,
      mode: "paused",
      reason: "Focused audited retirement regression.",
      owner_response: "Approved for test",
    });
    const recovered = await mcpTool<{
      ok: boolean;
