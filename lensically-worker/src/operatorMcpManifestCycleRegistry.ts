...
export const OPERATOR_MCP_MANIFEST_CYCLE_TOOLS: OperatorMcpToolDefinition[] = [
  ...
  },
  {
    name: "get_manifest_locked_lineup_page",
        ...
        description: "Return one bounded page of the exact backend-locked Main Cycle source-to-slot lineup with source evidence, active verbatim owner guidance, the latest relevant owner correction, source identities, and the persisted cycle_plan_item_id plus cycle_strategy_id when strategy is locked. Read from offset 0 until complete once per prepared cycle and again after strategy lock when bounded persistence requires exact plan identities. A source card is evidence, not a script. Do not replace locked sources and do not fetch individual source cards already represented in these pages.",
    inputSchema: {
      type: "object",
      properties: {
        brand_key: BRAND_KEY_SCHEMA,
  ...
  },
  {
    ...
    title: "Lock Manifest cycle strategy",
                                                description: "Using the compact versioned decision bundle returned by the Main Cycle prepare tool and the complete paged backend-locked lineup returned by get_manifest_locked_lineup_page, persist exactly one account-wide strategy without changing any source-to-slot assignment. The model must rank mature posts primarily by 24-hour likes, cite actual mature post IDs when mature evidence exists, separate uncertainty from conclusions, and may not create original posts, substitute sources, or form source-independent hypotheses. account_conclusion must include published_post_ids; it may be empty only when the snapshot has no mature posts, and otherwise must cite mature snapshot post IDs. Every strongest_executions and weakest_executions item must include published_post_ids and a non-empty reason. content_focus must include array fields emphasize, preserve, reduce, avoid_clustering, test, and unresolved_questions. Never include follower totals, follower velocity, follower projections, follower_context, or any other follower-shaped field anywhere in account_conclusion, content_focus, benchmarks, executions, directives, experiments, risks, or lineup; follower trajectory is account-level context outside the content-strategy payload and the server rejects it here. A bounded evidence-page detail read is allowed only when the bundle flags genuine ambiguity. Replaying an identical strategy is safe; a conflicting second strategy is blocked.",
    inputSchema: {
      type: "object",
      properties: {
        brand_key: BRAND_KEY_SCHEMA,
