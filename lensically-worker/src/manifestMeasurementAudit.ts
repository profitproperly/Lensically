...
export async function ensureManifestMeasurementAuditTables(db: D1Database): Promise<void> {
  ...
  await Promise.all([
    assertDatabaseIntegrity(db, {
      table: "operator_manifest_learning_briefs",
      columns: ["id", "brand_key", "brief_key", "brief_version", "source_fingerprint", "evidence_window_start", "evidence_window_end", "authoritative_post_count", "brief_json", "strategy_change_json", "strategy_version_id", "created_at", "updated_at"],
    }),
    assertDatabaseIntegrity(db, {
      table: "operator_manifest_benchmark_snapshots",
      columns: ["id", "brand_key", "snapshot_key", "cycle_id", "benchmark_version", "window_start", "window_end", "metrics_json", "source_fingerprint", "created_at", "updated_at"],
    }),
    assertDatabaseIntegrity(db, {
      table: "operator_manifest_run_comparisons",
      columns: ["id", "brand_key", "cycle_id", "previous_cycle_id", "comparison_version", "comparison_json", "source_fingerprint", "created_at", "updated_at"],
    }),
    assertDatabaseIntegrity(db, {
      table: "operator_manifest_saved_pattern_intelligence",
      columns: ["id", "brand_key", "pattern_identity_key", "external_pattern_id", "source_identity_key", "verified_metrics_json", "semantic_json", "mechanism_json", "adaptation_options_json", "similarity_json", "usage_json", "results_json", "confidence_json", "reuse_state", "exclusion_state", "source_updated_at", "intelligence_version", "created_at", "updated_at"],
    }),
    assertDatabaseIntegrity(db, {
      table: "operator_manifest_follower_checkpoints",
      columns: ["id", "brand_key", "checkpoint_key", "threads_user_id", "checkpoint_version", "snapshot_date", "followers_count", "follower_goal", "distance_to_goal", "trajectory_json", "attribution_policy", "created_at", "updated_at"],
    }),
  ]);
}
