...
const SOURCE_TRANSFORMATION_CONTRACT_SCHEMA = {
  type: "object",
  description: "Internal source-fidelity persistence and gate contract. Never expose these property names as owner-facing source-card headings. For Manifest, preserve the original hook, structure, meaning, tone, and payoff; close mimicry is preferred and transformation lists are optional.",
  properties: {
  ...
  return [
    ...
    "Proceed to the next step?",
            "Only after the user explicitly approves proceeding, call confirmOperatorProceed. If the direct tool is not exposed, call listMcpTools with execute_tool=confirmOperatorProceed and brand_key. Then include proceed_confirmed=true on account-scoped calls; use the bridge when a cached direct schema does not expose that field.",
                ...
                "Manifest close-mimicry rule: preserve the source hook, structure, meaning, tone, and payoff; make only slight wording changes; do not invent scenes, characters, activities, settings, events, metaphors, or premises. Exact source copying remains blocked.",
    "Source-card presentation rule: raw schema, transformation-contract, forbidden-surface, inventory-constraint, and pass/fail fields are internal persistence and gate data. Never display Must preserve, May reuse, Must change, Cannot repeat, Pass conditions, Fail conditions, or snake_case contract properties as owner-facing source-card headings. For Manifest, present natural sections covering Original source, Source performance, Why the source works, Audience reward, Adaptation approach, Generation freedom, only genuinely Necessary boundaries, and Recommended direction. Do not turn source-card review into a finished generated post.",
    ...
    "Owner-visible transition labels are mandatory across accounts and fresh chats: use brief explicit labels for Completed:, Showing now:, and Next decision:. Report the completed action before displaying the next artifact, and never make the owner infer whether they are viewing a source card, generated post, scheduling confirmation, or completion report.",
