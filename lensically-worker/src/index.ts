...
const OPERATOR_REJECTION_CONTEXT_VERSION = "operator-rejection-context-v2";
...
const DEFAULT_OPERATOR_GATES: Array<{
  ...
  { gate_key: "source_transformation_contract_gate", display_name: "Source Transformation Contract", description: "Draft must preserve approved exact hooks/functions, transform designated elements, deliver the audience reward, and avoid prohibited complete source packages.", stage_scope: "gate_evaluation", gate_type: "hard", severity: "block", evaluator: "backend", order_index: 15 },
  { gate_key: "source_surface_copy_gate", display_name: "Role-Aware Source Surface Copy", description: "Draft must avoid forbidden source surfaces except those explicitly approved for exact preservation or reuse.", stage_scope: "gate_evaluation", gate_type: "hard", severity: "block", evaluator: "backend", order_index: 20 },
    { gate_key: "current_inventory_repeat_gate", display_name: "Current Inventory Repeat", description: "Draft must not repeat the latest account opening or realm entrance too closely unless the active source contract explicitly requires that exact reusable hook.", stage_scope: "gate_evaluation", gate_type: "hard", severity: "block", evaluator: "backend", order_index: 30 },
  { gate_key: "historical_owner_rejection_gate", display_name: "Historical Owner Rejection", description: "Draft must be reviewed against the selected account's complete owner-rejection context and must not repeat banned surfaces, rejected concepts, failed structures, or semantically near-identical rejected drafts.", stage_scope: "gate_evaluation", gate_type: "hard", severity: "block", evaluator: "hybrid", order_index: 40 },
...
async function listSourceCandidatesForBrand(
  ...
  const wantsPatterns = sourceTypes.length === 0 || sourceTypes.includes("saved_pattern");
    ...
    return {
      candidates: qualifiedPool.slice(normalizedOffset, normalizedOffset + normalizedLimit),
      total_count: qualifiedPool.length,
    };
  }
...
const OPERATOR_MCP_TOOLS: OperatorMcpToolDefinition[] = [
  ...
  },
  {
    ...
    title: "Run operator gates",
                description: "Evaluate source-card or draft gates, including role-aware source preservation, transformation, full-package copying, reusable-hook exemptions, account-wide historical owner rejection review, and required-gate execution. Gate evaluation is a silent internal stage after source-card approval. For historical_owner_rejection_gate, submit auditable model evidence using the exact rejection-context fingerprint and reviewed count returned by create_generation_run. Missing rejection review or any missing blocking gate keeps showable=false.",
...
function operatorMcpInstructions(toolCount: number): string {
  return [
    ...
    "Rejection-memory rule: every generation run must load and persist the selected account's complete owner-rejection context. Review that context before drafting and use it during silent self-rejection so the owner never has to repeat an existing rejection lesson in a new chat.",
    "For historical_owner_rejection_gate, submit auditable model evidence containing the exact context_fingerprint and reviewed_rejection_count returned in account_rejection_context. Explicit banned surfaces, close matches to rejected drafts, partial review, a missing required gate, or a claimed gate that did not actually execute must keep showable=false.",
