...
function followerAttributionViolations(
  ...
  if (Array.isArray(value)) {
    ...
    const policyBoundaryPath = ["prohibit", "forbid", "disallow", "boundary", "policy", "constraint", "guard"]
    ...
    const explicitPolicyBoundary = /\b(?:do not|don't|does not|doesn't|never|must not|should not|cannot|can't)\s+(?:infer|claim|attribute|connect|tie|justify|support|establish|prove|permit|allow)\b.{0,120}\bfollowers?\b/.test(normalized)
    ...
    return explicitPositiveAttribution || (scopedFollowerClaim && !explicitPolicyBoundary && !policyBoundaryPath)
      ? [`${path}:text_claim`]
      : [];
  }
