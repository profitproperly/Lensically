import { LENSICALLY_NATIVE_RECOVERY_CONTRACT } from "./humanFreeAutonomy";
...
export const OPERATOR_MCP_VERSION = "1.45.11";
...
export const OPERATOR_GOVERNING_STANDARDS_VERSION = "operator-governing-standards-v11";
...
export const OPERATOR_ACTION_RULE_REGISTRY_VERSION = "operator-action-rule-registry-v5";
...
export const OPERATOR_DISCOVERY_EXECUTION_RULE = "Tool discovery, schema search, and keyword matching return candidates only; they never authorize execution. Before invoking a discovered tool, verify that its declared name, title, description, and side-effect class match the requested operation. Never invoke a mutating or business-execution tool for search, inspection, schema lookup, diagnosis, or explanation merely because discovery returned it. If the purpose does not match, do not call it; discover or invoke the correct read-only or engineering tool instead. Once the intended action is identified, treat Step 2 as research: inspect and reason under the bound prevention_rules, action_intelligence, winning-path evidence, and relevant failure history before advancing to Step 3 or Step 4. A returned binding is not authorization. If the applicable intelligence identifies a known losing path, unresolved blocker, wrong execution plane, invalid argument shape, or unproven exact repository path, veto the operational call and satisfy the prescribed prerequisite or repair first.";
...
export const OPERATOR_TURN_CLOSE_RULE = "A normal Operator turn may end only when no reachable next action remains, an explicit protected owner-only action is required, or a verified external dependency leaves no executable repair or continuation step. A blocker, failed tool, unresolved prevention obligation, active interrupt, or reachable next action forces continuation; blocked work enters repair mode and never terminal mode. Lifecycle action closure closes one action only and does not authorize conversational termination. Status reporting may describe progress but may not substitute for execution. Do not invent work after true terminal closure.";
export const OPERATOR_FAILURE_REPAIR_RULE = "If any Operator tool invocation or execution step exposes a failure, blocker, unintended result, failed assumption, avoidable retry, wrong route, stale-state use, validation miss, inefficient detour, or other deviation from intended execution, immediately interrupt the affected objective. There is no severity floor. Record the smallest complete evidence package, then before selecting a fix, retrieve prior failure and hardening intelligence for the same or adjacent signature. If the failure was previously encountered, treat recurrence as evidence that the prior repair, retrieval, enforcement, or prevention was insufficient; diagnose the escape and harden it again. If novel, record it as a new failure class before repair. Do not use altered arguments, alternate target forms, sibling business tools, or another business-execution path merely to accomplish the blocked action while leaving the failure unexplained. Switch into diagnosis and repair through Main's engineering, diagnostic, hardening, validation, and release capabilities while Main remains callable. Only when Main is unavailable or incapable may MCP Controller enter as the fleet break-glass plane; Controller repairs source, configuration, deployment, or runtime infrastructure, verifies Main exactly, and returns control to Main. The dedicated Lensically Recovery connector is retired and must not be selected. Identify and fix the exact root cause, record and generalize the lesson, add durable regression and prevention, validate the repair, deploy it when required, and then resume the original objective from the interrupted step. Alternate tools are required when they are part of root-cause repair; they are forbidden when they substitute for repairing the failed operation. An uncertain transport result may be reconciled with the same operation_id only to determine execution state.";
...
export const OPERATOR_CLIENT_PREDISPATCH_BLOCK_RULE = "When a client or provider rejects an Operator call before it reaches Lensically, do not infer a Lensically server defect and do not mutate Lensically server behavior solely from that rejection. Record the client block, prove the pre-dispatch boundary with server-receipt or telemetry evidence when available, and preserve the exact prepared action. Diagnose the client-side boundary using available hardening and telemetry; do not blindly retry, substitute, or reshape the blocked request merely to obtain a pass. When authoritative server receipt or telemetry proves the prior attempt did not execute, or stable idempotent state reconciliation proves replay is safe, a controlled verification replay of the same semantic operation is permitted under the current winning-path and prevention binding. Reprepare only lifecycle or transport state required by the current contract; do not switch to a sibling operation, alter semantics, weaken validation, or generate retry spam. Each failed replay remains evidence and keeps the incident open; when success remains reachable, a successful replay is required before closure. Repeated identical no-progress client blocks may be classified as an external dependency only after authoritative evidence shows the controlled replay cannot currently progress. A client-side block by itself never authorizes an MCP refresh or a new-chat instruction. Those are permitted only when the public-schema refresh rule independently proves that the current chat is surfacing a stale public schema and cannot use the newly deployed contract. A Lensically server repair requires server-side evidence; repeated client-side blocking remains a client/platform compatibility issue unless server-side evidence establishes otherwise.";
...
A tool failure interrupts the affected business task. Do not route around it to finish the action. Use Main engineering, diagnostic, hardening, validation, and release capabilities while Main is callable. If Main is unavailable or incapable, use MCP Controller as the fleet break-glass repair plane, verify Main exactly, return control to Main, then resume the interrupted step. The dedicated Lensically Recovery connector is retired.
...
export const OPERATOR_GOVERNING_STANDARDS = {
  ...
  native_recovery_contract: LENSICALLY_NATIVE_RECOVERY_CONTRACT,
