...
export const OPERATOR_OPAQUE_LIFECYCLE_TOKEN_RULE = "Lifecycle tokens are opaque signed values. Forward each lifecycle token byte-for-byte from the immediately preceding lifecycle response into the next required stage. Never decode, reconstruct, transcribe, normalize, trim, edit, or otherwise rewrite a lifecycle token. Treat every lifecycle token as a copy-only atomic value: never manually type or reconstruct any portion from decoded, remembered, summarized, or reasoned content. If the client or tool harness cannot bind or copy the exact returned string mechanically, stop before making the next lifecycle call instead of attempting manual transcription. If an invalid-or-expired rejection follows anything other than direct byte-for-byte forwarding, classify the event as client forwarding corruption, discard the corrupted token, record the root cause, and obtain a fresh token from the immediately preceding stage only after prevention is locked; never retry the corrupted token.";
