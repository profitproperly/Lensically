...
const OPERATOR_MCP_AUTONOMOUS_EXECUTION_BASE_TOOLS: OperatorMcpToolDefinition[] = [
  {
    name: "prepare_manifest_autonomous_cycle",
        title: "Prepare Manifest Main Cycle",
    description: "Invocation integrity: when a request explicitly names prepare_manifest_autonomous_cycle, call this tool immediately after it is available; tool discovery or schema loading is not execution, and no failure may be reported without this tool's returned result. This is the live Main Cycle only; it never invokes, connects, or converts the isolated Innovation Cycle. Refresh live Threads publications, use Threads and database clock evidence, reconcile scheduled, posting, posted, failed, stalled, retry-required, and manually published records, ignore every elapsed hour, preserve every valid existing post, and compute exactly the missing 0-through-48 slots inside the fixed 48-hour runway. Reuse fresh finalized intelligence when no due maturity checkpoint changed, return one compact decision bundle, then read the exact locked source lineup through get_manifest_locked_lineup_page. The model must inspect recent published posts, future scheduled exposure, repetition pressure, performance, Content Focus, and follower trajectory; sequence the complete actual deficit before persisting the first post; preserve franchise winners while spacing clustered execution; and justify why each family belongs in each exact slot. Reusing the same daily operation id refreshes durable Main Cycle state instead of replaying completed work. Default generation is eight source-backed candidates persisted through two persist_manifest_autonomous_batch calls; each batch reconciles authoritative coverage once, so do not call get_hourly_coverage or prepare a second cycle between accepted batches."
    inputSchema: {
      type: "object",
      properties: {
...
export const OPERATOR_MCP_AUTONOMOUS_EXECUTION_TOOLS: OperatorMcpToolDefinition[] = [
  ...
  persistManifestPostTool,
  {
    name: "persist_manifest_autonomous_batch",
        title: "Persist Manifest Main Cycle batch",
