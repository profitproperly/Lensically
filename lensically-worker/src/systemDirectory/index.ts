...
export const LENSICALLY_SYSTEM_DIRECTORY_ENTRIES: readonly SystemDirectoryEntry[] = [
  ...
  {
    id: "deployment.local_execution_node",
    title: "Local execution node validation plane",
    plane: "deployment",
    system_of_record: "operator_local_execution_nodes, operator_local_execution_jobs, operator_local_validation_receipts, and the Windows outbound polling node",
    primary_surfaces: ["local execution node", "validation plane router", "local validation receipts"],
    objects: ["local node", "validation job", "exact-SHA receipt", "worker update", "execution plane"],
    keywords: ["local execution node", "local validation", "validation plane", "local worker update", "validate sha locally"],
    capabilities: ["read local execution status", "queue exact-SHA validation", "route validation plane", "deploy locally validated SHA", "update local worker"],
    payload: { action_size: "bounded_mutation", max_response_bytes: 14000, required_inputs: ["commit_sha"] },
    route_intent: "validation plane execute",
    related_entry_ids: ["engineering.operator_validation", "deployment.main_worker", "engineering.repository"],
    recommended_next_planes: ["deployment", "engineering"],
    hard_gates: [
      "Local node is the primary free validation plane when heartbeat is healthy.",
      "Hosted fallbacks are used only for local unavailability, infrastructure failure, or explicit independent verification.",
      "Code/test failures must be repaired and must not be hidden by switching planes.",
      "Deployment requires requested SHA, checked-out SHA, validated SHA, receipt SHA, and release candidate SHA to match.",
    ],
  },
  {
    id: "deployment.local_execution_status",
    title: "Local execution node status",
    plane: "deployment",
    system_of_record: "operator_local_execution_nodes and operator_local_execution_jobs",
    primary_surfaces: ["local execution node", "validation plane router"],
    objects: ["local node status", "heartbeat", "queue counts", "local validation receipt"],
    keywords: ["local execution status", "local node heartbeat", "validation plane status"],
    capabilities: ["read local execution status"],
    payload: { action_size: "bounded_read", max_response_bytes: 14000 },
    route_intent: "local execution status",
    related_entry_ids: ["deployment.local_execution_node", "engineering.operator_validation"],
    recommended_next_planes: ["deployment"],
  },
  {
    id: "deployment.local_execution_validate_sha",
    title: "Local exact-SHA validation",
    plane: "deployment",
    system_of_record: "operator_local_execution_jobs and local-validation-receipt-v1",
    primary_surfaces: ["local execution node", "validation queue"],
    objects: ["validate_sha job", "exact SHA", "validation receipt"],
    keywords: ["local execution validate sha", "validate sha locally", "exact sha validation"],
    capabilities: ["queue exact-SHA validation"],
    payload: { action_size: "bounded_mutation", max_response_bytes: 12000, required_inputs: ["commit_sha"] },
    route_intent: "local execution validate sha",
    related_entry_ids: ["deployment.local_execution_node", "engineering.repository"],
    recommended_next_planes: ["deployment", "engineering"],
    hard_gates: ["Only exact 40-character repository SHAs may be queued."],
  },
  {
    id: "deployment.local_execution_full_validation",
    title: "Local full validation",
    plane: "deployment",
    system_of_record: "operator_local_execution_jobs and local-validation-receipt-v1",
    primary_surfaces: ["local execution node", "validation queue"],
    objects: ["run_full_validation job", "full validation", "exact SHA"],
    keywords: ["local execution full validation", "run full validation locally"],
    capabilities: ["queue full local validation"],
    payload: { action_size: "bounded_mutation", max_response_bytes: 12000, required_inputs: ["commit_sha"] },
    route_intent: "local execution full validation",
    related_entry_ids: ["deployment.local_execution_node", "engineering.operator_validation"],
    recommended_next_planes: ["deployment", "engineering"],
    hard_gates: ["Full validation must remain bound to one exact repository SHA."],
  },
  {
    id: "deployment.local_execution_deploy_validated_sha",
    title: "Local validated SHA deployment queue",
    plane: "deployment",
    system_of_record: "operator_local_validation_receipts and operator_local_execution_jobs",
    primary_surfaces: ["local execution node", "local validation receipts", "deployment queue"],
    objects: ["deploy_validated_sha job", "local validation receipt", "release candidate SHA"],
    keywords: ["local execution deploy validated sha", "deploy locally validated sha"],
    capabilities: ["deploy locally validated SHA"],
    payload: { action_size: "bounded_mutation", max_response_bytes: 12000, required_inputs: ["commit_sha", "receipt_id"] },
    route_intent: "local execution deploy validated sha",
    related_entry_ids: ["deployment.local_execution_node", "deployment.main_worker"],
    recommended_next_planes: ["deployment"],
    hard_gates: ["A matching successful local-validation-receipt-v1 is required before any deploy job is queued."],
  },
  {
    id: "deployment.local_execution_update_worker",
    title: "Local execution worker update",
    plane: "deployment",
    system_of_record: "Windows local execution node active and previous worker slots",
    primary_surfaces: ["local execution node", "worker slot manager"],
    objects: ["update_worker job", "active slot", "inactive slot", "worker package"],
    keywords: ["local execution update worker", "local worker update", "inactive slot"],
    capabilities: ["update local worker"],
    payload: { action_size: "bounded_mutation", max_response_bytes: 12000, required_inputs: ["commit_sha", "worker_version", "package_hash"] },
    route_intent: "local execution update worker",
    related_entry_ids: ["deployment.local_execution_node"],
    recommended_next_planes: ["deployment"],
    hard_gates: ["Ordinary worker updates must install into the inactive slot and cannot change signing authority."],
  },
  {
    id: "deployment.local_execution_cancel_job",
    title: "Local execution job cancellation",
    plane: "deployment",
    system_of_record: "operator_local_execution_jobs",
    primary_surfaces: ["local execution node", "validation queue"],
    objects: ["local execution job", "cancelled job"],
    keywords: ["local execution cancel job", "cancel local validation job"],
    capabilities: ["cancel local execution job"],
    payload: { action_size: "bounded_mutation", max_response_bytes: 8000, required_inputs: ["job_id"] },
    route_intent: "local execution cancel job",
    related_entry_ids: ["deployment.local_execution_node"],
    recommended_next_planes: ["deployment"],
  },
  {
    id: "deployment.local_execution_enrollment",
    title: "Local execution node enrollment",
    plane: "deployment",
    system_of_record: "operator_local_execution_enrollment_tokens and per-node credential hashes",
    primary_surfaces: ["local execution node", "enrollment token issuer"],
    objects: ["enrollment token", "node credential", "credential hash", "node identity"],
    keywords: ["local execution enrollment", "enroll local execution node", "single use token"],
    capabilities: ["create local node enrollment token"],
    payload: { action_size: "bounded_mutation", max_response_bytes: 8000, required_inputs: ["node_id"] },
    route_intent: "local execution enrollment",
    related_entry_ids: ["deployment.local_execution_node"],
    recommended_next_planes: ["deployment"],
    hard_gates: ["Enrollment tokens are short-lived and single-use; Main stores only the per-node credential hash."],
  },
  {
    id: "deployment.local_execution_revoke_node",
    title: "Local execution node revocation",
    plane: "deployment",
    system_of_record: "operator_local_execution_nodes credential status",
    primary_surfaces: ["local execution node", "credential registry"],
    objects: ["node credential", "revoked node", "credential status"],
    keywords: ["local execution revoke node", "revoke local node credential"],
    capabilities: ["revoke local execution node"],
    payload: { action_size: "bounded_mutation", max_response_bytes: 8000, required_inputs: ["node_id"] },
    route_intent: "local execution revoke node",
    related_entry_ids: ["deployment.local_execution_node", "deployment.local_execution_status"],
    recommended_next_planes: ["deployment"],
  },
  {
    id: "deployment.validation_plane_status",
    title: "Validation plane status",
    plane: "deployment",
    system_of_record: "local execution node status, Cloudflare Builds fallback state, and GitHub Actions fallback policy",
    primary_surfaces: ["validation plane router", "local execution node"],
    objects: ["validation plane", "Cloudflare Builds fallback", "GitHub Actions fallback"],
    keywords: ["validation plane status", "cloudflare builds fallback", "github actions fallback"],
    capabilities: ["read validation plane status"],
    payload: { action_size: "bounded_read", max_response_bytes: 10000 },
    route_intent: "validation plane status",
    related_entry_ids: ["deployment.local_execution_node", "deployment.main_worker"],
    recommended_next_planes: ["deployment", "engineering"],
  },
  {
    ...
    plane: "deployment",
        system_of_record: "Local execution node receipts, Cloudflare Builds fallback, verified source marker, and Cloudflare Worker versions",
    primary_surfaces: ["Local execution node", "Cloudflare Worker deployment", "Lensically Validation workflow"],
    objects: ["deployment", "Worker version", "release", "live commit"],
    keywords: ["deploy", "release", "ship", "live worker", "exact SHA"],
    capabilities: ["verify repository head", "validate exact SHA through the selected plane", "commit Main release marker", "verify live version"],
    payload: { action_size: "coherent_change_set", max_response_bytes: 12000 },
        related_entry_ids: ["deployment.local_execution_node", "engineering.repository", "engineering.operator_validation", "recovery.control_plane"],
    recommended_next_planes: ["engineering"],
    hard_gates: ["Main releases require a verified repository head and an exact-SHA validation receipt.", "Local execution is preferred; Cloudflare Builds is second and GitHub Actions is third.", "Recovery remains break-glass only."],
  },
  {
