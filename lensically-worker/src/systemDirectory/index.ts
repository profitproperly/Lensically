...
export const LENSICALLY_SYSTEM_DIRECTORY_ENTRIES: readonly SystemDirectoryEntry[] = [
  ...
  },
  {
    ...
    title: "Published lineage integrity",
    plane: "content_production",
    ...
    objects: ["winner lineage", "historical published post", "lineage audit", "source-card backfill", "Saved Pattern recovery"],
    keywords: ["audit winner lineage", "verify source to metrics lineage", "recover winner lineage", "backfill source card", "tie winner to saved pattern", "restore post lineage"],
    capabilities: ["audit published post lineage", "recover published post lineage", "preserve historical metrics", "create dedicated recovery generation runs"],
    payload: { action_size: "bounded_operation", max_results: 50, max_response_bytes: 22000, required_inputs: ["brand_key"] },
    route_intent: "audit published post lineage",
    related_entry_ids: ["content.sources", "content.production", "analytics.performance_learning", "product.post_archive"],
    recommended_next_planes: ["analytics", "content_production"],
    hard_gates: [
