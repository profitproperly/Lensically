...
export const LENSICALLY_SYSTEM_DIRECTORY_ENTRIES: readonly SystemDirectoryEntry[] = [
  ...
  },
  {
    ...
    capabilities: ["lock exactly one strategy per cycle", "require mature evidence citations", "cover every missing slot", "bind every plan item to a canonical source card"],
    payload: { action_size: "bounded_mutation", max_results: 72, max_response_bytes: 24000, required_inputs: ["brand_key", "cycle_id", "snapshot_id", "account_conclusion", "content_focus", "benchmarks", "lineup"] },
    route_intent: "commit manifest cycle strategy",
    related_entry_ids: ["strategy.manifest_cycle_analysis_page", "content.manifest_autonomous_post_persistence", "content.sources"],
    recommended_next_planes: ["content_production"],
    hard_gates: ["Every evidence page must be consumed.", "Exactly one strategy is allowed per cycle.", "Original model sources are forbidden.", "Every authoritative missing slot requires a canonical source-card-backed plan item."],
  },
  {
    ...
    capabilities: ["persist exactly one planned post", "verify cycle strategy and source identity", "execute nonempty candidate gates", "preserve full evidence-to-metrics lineage", "resume one identical post idempotently"],
    payload: { action_size: "single_record", max_results: 1, max_response_bytes: 16000, required_inputs: ["brand_key", "cycle_id", "cycle_strategy_id", "plan_item_id", "post", "model_evaluation", "operation_id"] },
    route_intent: "persist manifest autonomous post",
    related_entry_ids: ["strategy.manifest_cycle_strategy", "content.production", "publishing.scheduled_posts", "analytics.performance_learning"],
    recommended_next_planes: ["publishing", "analytics"],
    hard_gates: ["Only the exact locked plan item for the authoritative missing slot may be persisted.", "The post must use its canonical source card and may not originate from the model.", "A nonempty passing owner-rule and backend gate receipt is mandatory.", "Every accepted post requires evidence-to-strategy-to-plan-to-source-to-gate lineage."],
  },
    {
