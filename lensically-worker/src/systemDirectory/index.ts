...
export const LENSICALLY_SYSTEM_DIRECTORY_ENTRIES: readonly SystemDirectoryEntry[] = [
  ...
  },
  {
    ...
    title: "Operator action execution",
    plane: "workflows",
    system_of_record: "executeOperatorAction and the existing registered-profile Execution Kernel",
    primary_surfaces: ["Step-4 action gateway", "Execution Kernel", "registered profile routing"],
    objects: ["operator action", "execute action", "registered capability", "action receipt"],
    keywords: ["execute", "action", "Step 4", "registered profile", "Execution Kernel"],
    capabilities: ["execute one registered action", "enforce live-state proof", "preserve typed capability routing and idempotency"],
        payload: { action_size: "bounded_mutation", max_response_bytes: 24000, required_inputs: ["live_state_token", "action"] },
    route_intent: "execute operator action",
    related_entry_ids: ["operating.live_state", "operating.action_closure"],
    recommended_next_planes: ["workflows"],
    hard_gates: ["Step 4 cannot compile or execute a profile without valid current Step-3 proof; execution remains open until Step 5 closes it."],
  },
  {
