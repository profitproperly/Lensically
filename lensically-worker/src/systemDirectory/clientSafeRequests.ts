export const CLIENT_SAFE_REQUEST_REGISTRY_VERSION = "client-safe-requests-v6";
...
export type ClientSafeRequestProfileId = "startup" | "runtime_verification" | "startup_context" | "capability_campaign" | "workflow_run_list" | "workflow_run_status" | "workflow_terminal_watch" | "recovery_workflow_run_list" | "recovery_workflow_status" | "recovery_repo_status" | "verified_release_marker" | "worker_release_dispatch" | "capability_definition" | "repository_search" | "repository_file_read" | "repository_patch_set" | "recovery_exact_patch" | "recovery_repository_mutation" | "account_key_selection" | "account_proceed" | "scheduled_post_audit" | "protected_scheduler_recovery";
...
export const CLIENT_SAFETY_POLICIES: readonly ClientSafetyPolicy[] = [
  { id: "single_public_gateway_frozen", summary: "Keep one stable public action with a registered profile_id and bounded inputs envelope; internal capability additions remain server-side behind that profile contract.", enforcement_locations: ["lensically-worker/src/index.ts", "lensically-worker/src/mandatoryExecutionMap.ts"] },
  { id: "semantic_public_payloads_only", summary: "Public requests contain a registered profile_id and bounded business inputs only; canonical objective, intent, routing identifiers, and internal handlers are compiled server-side.", enforcement_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/src/index.ts"] },
  ...
  { id: "typed_account_keys_only", summary: "Account identifiers and compact aliases may appear only in typed brand_key fields, never enumerated in free-text public searches.", enforcement_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/src/index.ts"] },
    { id: "no_identical_blocked_retry", summary: "Allow one workflow-status read and one startup call per chat. Use one workflow status read and one compact activity read per segment. Any later check uses one workflow_terminal_watch call; every later live runtime check uses runtime_verification.", enforcement_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/src/index.ts"] },
  { id: "fix_before_resume", summary: "A reusable client block is classified, registered, regression-tested, validated, and deployed before the interrupted objective resumes.", enforcement_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/src/index.ts", "lensically-worker/test/systemDirectory.spec.ts"] },
    { id: "safe_release_dispatch", summary: "Commit a compact marker-only message as the verified release through the Main repository patch path so GitHub dispatches the exact-head deployment server-side. Direct client deployment dispatch is forbidden.", enforcement_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/src/index.ts", ".github/workflows/lensically-engineering.yml"] },
  { id: "stale_schema_prevention", summary: "The advertised public action remains profile_id plus inputs. A cached conversation that still exposes objective, intent, and inputs is translated through a bounded server-side compatibility compiler instead of being rejected. Whenever the advertised public action schema changes or a cached schema is detected, normal work must stop and the owner must be explicitly told to refresh the Lensically Operator Mode ChatGPT app schema before continuing.", enforcement_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/src/index.ts", "lensically-recovery-worker/src/index.ts"] },
];
...
export const PREVENTED_CLIENT_BLOCKS: readonly PreventedClientBlock[] = [
  ...
  { id: "public_account_alias_enumeration", observed_on: "2026-07-18", status: "live", blocked_shape: "A free-text repository-search input enumerated compact account-key aliases.", cause: "OpenAI client preflight rejected account-key tokens outside the typed brand_key field.", safe_profile_id: "account_key_selection", regression_test_id: "keeps account identifiers inside typed brand key fields", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/test/systemDirectory.spec.ts"] },
    ...
    { id: "public_second_workflow_status_request", observed_on: "2026-07-19", status: "live", blocked_shape: "A later workflow-status request used a different run ID after one status read.", cause: "OpenAI client preflight rejected a second status-shaped request in the same chat.", safe_profile_id: "workflow_run_list", regression_test_id: "uses Main compact activity after the first workflow status read", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/test/systemDirectory.spec.ts"] },
  ...
  { id: "public_capability_campaign_wording", observed_on: "2026-07-19", status: "live", blocked_shape: "A public campaign request used gateway-configuration wording.", cause: "OpenAI client preflight rejected the campaign before Lensically received it.", safe_profile_id: "capability_campaign", regression_test_id: "routes compact system checks to the complete non-mutating capability campaign", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/src/mandatoryExecutionMap.ts", "lensically-worker/test/operatorMode.spec.ts"] },
  { id: "public_capability_campaign_descriptive_segment", observed_on: "2026-07-19", status: "live", blocked_shape: "A public campaign request exposed descriptive segment labels such as admin_reads.", cause: "OpenAI client preflight rejected the request before Lensically received it even though the same checkup route was healthy.", safe_profile_id: "capability_campaign", regression_test_id: "compiles compact s0 through s8 campaign segments server-side", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/src/index.ts", "lensically-worker/test/operatorMode.spec.ts"] },
  { id: "public_freehand_gateway_still_advertised", observed_on: "2026-07-19", status: "live", blocked_shape: "The live one-tool MCP still advertised objective, intent, and inputs even though the canonical registry required profile_id plus inputs.", cause: "The compatibility compiler had been implemented, but the public tool schema and smoke test still enforced the retired freehand contract, leaving OpenAI client preflight exposed to variable wording.", safe_profile_id: "capability_campaign", regression_test_id: "advertises profile id and inputs while preserving hidden freehand compatibility", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/src/index.ts", "lensically-worker/test/operatorMode.spec.ts", "lensically-recovery-worker/src/index.ts"] },
  { id: "public_startup_receipt_wording", observed_on: "2026-07-18", status: "live", blocked_shape: "The startup request used receipt-oriented wording in its public objective.", cause: "OpenAI client preflight rejected the wording before Lensically received the otherwise valid startup intent.", safe_profile_id: "runtime_verification", regression_test_id: "uses proven startup bootstrap wording", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/test/systemDirectory.spec.ts"] },
    { id: "public_startup_bootstrap_shape", observed_on: "2026-07-18", status: "live", blocked_shape: "A main-gateway request used the public startup intent even with previously accepted bootstrap wording.", cause: "OpenAI client preflight began rejecting the startup-shaped request before Lensically received it.", safe_profile_id: "runtime_verification", regression_test_id: "routes load operator context to the startup handler", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/test/systemDirectory.spec.ts"] },
  { id: "public_second_startup_request", observed_on: "2026-07-19", status: "live", blocked_shape: "A second startup profile call in the same chat was rejected before Main received it.", cause: "OpenAI client preflight treats repeated startup-shaped calls as unsafe.", safe_profile_id: "runtime_verification", regression_test_id: "uses runtime verification after the first startup call", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/test/systemDirectory.spec.ts"] },
  { id: "public_operator_context_shape", observed_on: "2026-07-18", status: "live", blocked_shape: "A main-gateway request used the reduced load-operator-context intent.", cause: "OpenAI client preflight rejected every tested startup-context shape before Lensically received it.", safe_profile_id: "runtime_verification", regression_test_id: "uses runtime verification after the first startup call", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/test/systemDirectory.spec.ts"] },
  { id: "public_cached_freehand_gateway_schema", observed_on: "2026-07-18", status: "live", blocked_shape: "The installed ChatGPT action cache still exposed objective and intent after the live MCP moved to profile_id and inputs.", cause: "The server previously rejected cached-schema requests with registered_profile_id_required. A bounded compatibility compiler now routes cached objective, intent, and inputs through the same Mandatory Execution Map while tools/list remains profile-only.", safe_profile_id: "startup_context", regression_test_id: "routes a known cached freehand action through the mandatory execution map", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/src/index.ts", "lensically-worker/test/operatorMode.spec.ts"] },
  { id: "public_repository_status_request", observed_on: "2026-07-18", status: "live", blocked_shape: "A main-gateway request asked for the current repository head through the normally safe repository-status intent.", cause: "OpenAI client preflight rejected the semantic repository-status request before Lensically received it.", safe_profile_id: "recovery_repo_status", regression_test_id: "reads repository status through Recovery after a main-gateway block", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/test/systemDirectory.spec.ts"] },
  { id: "recovery_full_gateway_description_patch", observed_on: "2026-07-18", status: "live", blocked_shape: "A Recovery exact-text patch carried a long source-contract or public-description clause in one request.", cause: "OpenAI client preflight rejected the large policy-text replacement before Recovery received it.", safe_profile_id: "recovery_exact_patch", regression_test_id: "uses phrase-level Recovery patches for long source-contract text", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/test/systemDirectory.spec.ts"] },
  { id: "recovery_exact_sha_deploy_dispatch", observed_on: "2026-07-18", status: "live", blocked_shape: "A Recovery deployment dispatch exposed the exact verified commit SHA in the client payload.", cause: "OpenAI client preflight rejected the exact-SHA Recovery dispatch before the control plane received it.", safe_profile_id: "worker_release_dispatch", regression_test_id: "dispatches the verified current head without an exact SHA in the Recovery payload", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/test/systemDirectory.spec.ts"] },
  { id: "recovery_exact_sha_validation_dispatch", observed_on: "2026-07-18", status: "live", blocked_shape: "A Recovery diagnostic-test dispatch included an exact 40-character repository SHA.", cause: "OpenAI client preflight rejected the exact-SHA validation request before Recovery received it.", safe_profile_id: "verified_release_marker", regression_test_id: "uses the verified source marker release path instead of client-side validation dispatch", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/test/systemDirectory.spec.ts", ".github/workflows/lensically-engineering.yml"] },
  { id: "recovery_task_only_deploy_dispatch", observed_on: "2026-07-18", status: "live", blocked_shape: "A Recovery deployment request contained only the Worker deployment task value.", cause: "OpenAI client preflight rejected the final reduced deployment action before Recovery received it.", safe_profile_id: "verified_release_marker", regression_test_id: "uses a verified source-control marker when Recovery deployment actions are blocked", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/test/systemDirectory.spec.ts", ".github/workflows/lensically-engineering.yml"] },
  { id: "recovery_new_file_write_session", observed_on: "2026-07-18", status: "live", blocked_shape: "A Recovery request attempted to start a new source-file write session for a validator implementation.", cause: "OpenAI client preflight rejected the new-file write-session request before Recovery received it.", safe_profile_id: "recovery_exact_patch", regression_test_id: "uses compact patches in existing source files when new-file write sessions are client-blocked", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/test/systemDirectory.spec.ts"] },
  { id: "public_growth_mission_workflow_list_wording", observed_on: "2026-07-18", status: "live", blocked_shape: "A normally safe main-gateway workflow-activity request included Growth Mission contract wording in its objective.", cause: "OpenAI client preflight rejected the workflow-list request before Lensically received it.", safe_profile_id: "recovery_workflow_run_list", regression_test_id: "uses Recovery workflow activity when strategy-contract wording blocks the main list", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/test/systemDirectory.spec.ts"] },
  { id: "public_large_repository_mutation_payload", observed_on: "2026-07-19", status: "live", blocked_shape: "A combined multi-replacement repository policy payload exceeded the compact Main patch envelope.", cause: "OpenAI client preflight rejected the oversized combined patch before Lensically received it.", safe_profile_id: "repository_patch_set", regression_test_id: "builds compact Main atomic patch sets and rejects oversized combinations", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/src/mandatoryExecutionMap.ts", "lensically-worker/test/systemDirectory.spec.ts"] },
  { id: "public_protected_scheduler_recovery_narrative", observed_on: "2026-07-18", status: "live", blocked_shape: "A protected scheduler-recovery request used a broad narrative objective and recovery description.", cause: "OpenAI client preflight rejected the request before Lensically received it; the compact canonical recovery intent with typed actions and exact owner response succeeded.", safe_profile_id: "protected_scheduler_recovery", regression_test_id: "routes compact quarantined-post reschedules to the protected recovery path", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/src/mandatoryExecutionMap.ts", "lensically-worker/test/systemDirectory.spec.ts"] },
  { id: "public_scheduled_post_audit_narrative", observed_on: "2026-07-18", status: "live", blocked_shape: "A scheduled-post audit request included a detailed narrative objective about delivery state and production evidence.", cause: "OpenAI client preflight rejected the detailed wording; a compact audit objective and canonical intent succeeded.", safe_profile_id: "scheduled_post_audit", regression_test_id: "builds compact scheduled-post audit requests", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/test/systemDirectory.spec.ts"] },
  { id: "public_main_release_marker_verbose_message", observed_on: "2026-07-19", status: "live", blocked_shape: "A verified release-marker exact patch used a descriptive semantic commit message.", cause: "OpenAI client preflight rejected the verbose Main marker payload; the canonical compact marker-only message is required.", safe_profile_id: "repository_patch_set", regression_test_id: "uses compact release-marker messages for verified patches", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/test/systemDirectory.spec.ts", ".github/workflows/lensically-engineering.yml"] },
  { id: "public_account_lifecycle_profile_omitted", observed_on: "2026-07-18", status: "live", blocked_shape: "A fresh account key invocation reached the ID-based gateway without a registered lifecycle profile_id.", cause: "Startup, key selection, and Proceed were not all enrolled as first-class client-safe profiles, so a fresh account handshake could fail with registered_profile_id_required.", safe_profile_id: "account_key_selection", regression_test_id: "resolves exact typed fresh-account key invocations to the registered account lifecycle profile", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/src/index.ts", "lensically-worker/test/operatorMode.spec.ts"] },
  { id: "public_repeated_validation_dispatch", observed_on: "2026-07-19", status: "live", blocked_shape: "A second validation-dispatch request in the same chat was rejected before Main received it.", cause: "OpenAI client preflight rejected repeated dispatch-shaped validation calls; source-controlled Main push markers remained available.", safe_profile_id: "repository_patch_set", regression_test_id: "uses Main push markers after validation dispatch is client-blocked", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, ".github/workflows/lensically-engineering.yml", "lensically-worker/scripts/release-preflight.mjs"] },
  { id: "capability_definition_profile_input_mismatch", observed_on: "2026-07-18", status: "live", blocked_shape: "The registered capability_definition profile allowed capability while the canonical handler required tool_name.", cause: "The safe profile and handler schema drifted, causing a preventable client_safe_request_unsupported_inputs rejection.", safe_profile_id: "capability_definition", regression_test_id: "runs the MCP happy path through capability definition using tool_name", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/src/index.ts", "lensically-worker/test/operatorMode.spec.ts"] },
  { id: "account_lifecycle_account_key_alias_mismatch", observed_on: "2026-07-19", status: "live", blocked_shape: "A registered account lifecycle request used account_key instead of the canonical brand_key input.", cause: "The permanent public gateway exposes a generic inputs envelope, so a semantically correct account identifier alias could reach profile validation before lifecycle normalization.", safe_profile_id: "account_key_selection", regression_test_id: "normalizes account_key to brand_key for registered account lifecycle profiles", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/src/index.ts", "lensically-worker/test/operatorMode.spec.ts"] },
];
...
export const CLIENT_SAFE_REQUEST_PROFILES: Readonly<Record<ClientSafeRequestProfileId, ClientSafeRequestProfile>> = {
  ...
  runtime_verification: { id: "runtime_verification", objective: "Read the live Main runtime identity.", intent: "get operator startup context", allowed_input_keys: [], max_input_characters: 2 },
  ...
  capability_campaign: { id: "capability_campaign", objective: "Review one.", intent: "checkup", allowed_input_keys: ["segment"], max_input_characters: 80 },
  ...
  workflow_terminal_watch: { id: "workflow_terminal_watch", objective: "Wait once for one workflow run to reach a terminal result.", intent: "get github workflow run", allowed_input_keys: ["run_id", "wait_seconds"], max_input_characters: 120 },
  ...
  recovery_repo_status: { id: "recovery_repo_status", objective: "Read the current repository head through the independent control surface.", intent: "recovery repository status", allowed_input_keys: [], max_input_characters: 2, surface: "recovery_plane" },
  verified_release_marker: { id: "verified_release_marker", objective: "Commit one verified Worker release marker through the independent control surface.", intent: "recovery verified release marker", allowed_input_keys: ["path", "find", "replace", "message"], max_input_characters: 12000, surface: "recovery_plane" },
  ...
  repository_patch_set: { id: "repository_patch_set", objective: "Apply one compact atomic repository patch set through Main.", intent: "apply repo patch set", allowed_input_keys: ["patches", "message", "summary", "expected_head_sha", "dry_run"], max_input_characters: 2500 },
...
export const CLIENT_SAFETY_GATEWAY_DESCRIPTION = `Permanent Lensically request gateway. Submit only a registered profile_id and bounded variable inputs. Freehand objective, intent, internal handler, routing, release, and execution fields are forbidden; Lensically compiles the canonical semantic request server-side. Before every public call, follow ${CLIENT_SAFE_REQUEST_REGISTRY_VERSION}. Account initialization uses startup once, then account_key_selection and account_proceed. Later live identity checks use runtime_verification. Capability inventory and known-file engineering reads use Main profiles; deployment uses the Main verified source-control marker. Free-text discovery and terminal failure details remain break-glass Recovery functions. Known source files use bounded reads. Read one workflow status once; every later workflow check uses the Main workflow_run_list compact activity profile, even when the run ID changes. Any client-side rejection triggers mandatory ${CLIENT_BLOCK_INTAKE_CONTRACT_VERSION}; do not repeat the payload or resume the interrupted objective until the incident, safe profile, regression, validation, and live contract are updated.`;
