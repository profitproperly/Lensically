...
export const CLIENT_SAFETY_POLICIES: readonly ClientSafetyPolicy[] = [
  ...
  { id: "fix_before_resume", summary: "A reusable client block is classified, registered, regression-tested, validated, and deployed before the interrupted objective resumes.", enforcement_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/src/index.ts", "lensically-worker/test/systemDirectory.spec.ts"] },
    { id: "safe_release_dispatch", summary: "Verify repository head through Recovery, then commit the verified release marker with a compact marker-only message so GitHub dispatches deployment server-side. Direct client deployment actions and descriptive release-marker messages are forbidden.", enforcement_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-recovery-worker/src/index.ts", ".github/workflows/lensically-engineering.yml"] },
...
export const PREVENTED_CLIENT_BLOCKS: readonly PreventedClientBlock[] = [
  ...
  { id: "recovery_task_only_operator_validation_dispatch", observed_on: "2026-07-18", status: "live", blocked_shape: "A Recovery validation request contained only task=operator-tests.", cause: "OpenAI client preflight rejected the previously accepted task-only validation dispatch before Recovery received it.", safe_profile_id: "verified_release_marker", regression_test_id: "uses the verified source marker release path when a task-only Recovery validation dispatch is blocked", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, ".github/workflows/lensically-engineering.yml", "lensically-worker/scripts/release-preflight.mjs"] },
