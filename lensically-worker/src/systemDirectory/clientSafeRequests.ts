...
export type ClientSafeRequestProfileId = "startup_context" | "workflow_run_list" | "workflow_run_status" | "recovery_workflow_run_list" | "recovery_workflow_status" | "recovery_repo_status" | "validation_dispatch" | "verified_release_marker" | "worker_release_dispatch" | "capability_definition" | "repository_search" | "repository_file_read" | "recovery_exact_patch" | "recovery_repository_mutation" | "account_key_selection" | "scheduled_post_audit" | "protected_scheduler_recovery";
...
export const CLIENT_SAFETY_GATEWAY_DESCRIPTION = `Permanent Lensically request gateway. Submit objective, intent, and bounded variable inputs only. Before every public call, follow ${CLIENT_SAFE_REQUEST_REGISTRY_VERSION}. Do not submit startup-context, capability-inventory, or repository-status requests through this gateway; read them through the independent Recovery control surface. Never submit deployment requests or free-text repository discovery through this gateway; verify repository head through Recovery and commit the verified release marker so GitHub dispatches deployment server-side. Known source files may use bounded direct reads. Read one workflow status once; for later checks use compact recent activity or change the server-bounded request. When workflow-list wording contains strategy or Growth Mission contract language, use the Recovery workflow list. Any client-side rejection triggers mandatory ${CLIENT_BLOCK_INTAKE_CONTRACT_VERSION}; do not repeat the payload or resume the interrupted objective until the registry, safe profile, regression, validation, and live contract are updated.`;
