...
export const CLIENT_SAFETY_POLICIES: readonly ClientSafetyPolicy[] = [
  ...
  { id: "stale_schema_prevention", summary: "The installed public action must expose profile_id and inputs and must not expose freehand objective or intent fields. A conversation holding the stale schema is prohibited from calling the main gateway until the app action is refreshed.", enforcement_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-worker/src/index.ts", "lensically-recovery-worker/src/index.ts"] },
];
...
export const PREVENTED_CLIENT_BLOCKS: readonly PreventedClientBlock[] = [
  ...
  { id: "public_cached_freehand_gateway_schema", observed_on: "2026-07-18", status: "live", blocked_shape: "The installed ChatGPT action cache still exposed objective and intent after the live MCP moved to profile_id and inputs.", cause: "The current conversation retained a stale MCP tool schema even though the live tools/list contract had removed the freehand fields.", safe_profile_id: "startup_context", regression_test_id: "records and forbids cached freehand gateway schemas", source_locations: [CLIENT_SAFETY_CANONICAL_LOCATION, "lensically-recovery-worker/src/index.ts", "lensically-worker/test/systemDirectory.spec.ts"] },
...
export const CLIENT_SAFE_REQUEST_PROFILES: Readonly<Record<ClientSafeRequestProfileId, ClientSafeRequestProfile>> = {
  ...
  capability_definition: { id: "capability_definition", objective: "Read the compact definition for the named semantic capability.", intent: "read capability definition", allowed_input_keys: ["tool_name"], max_input_characters: 160 },
