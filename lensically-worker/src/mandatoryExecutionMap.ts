...
export const WINNING_PATH_PROMOTIONS: readonly WinningPathPromotion[] = [
  ...
  },
  {
    ...
    losing_path: "Expose semantic hardening control-plane names or transition fields directly to the client, or allow the neutral case-step prevention to exist only as closure text without action binding.",
    winning_path: {
      surface: "main_gateway",
      route_intent: "advance hardening case",
      procedure: ["Expose the neutral case_step capability and neutral evidence aliases only.", "Compile neutral fields server-side into advanceHardeningIncident semantics.", "Bind this prevention to advanceHardeningIncident before Step 4.", "Require the normal hardening validator and explicit lifecycle closure."],
    },
    evidence: ["OpenAI pre-dispatch blocking previously rejected semantic hardening_transition forms while the neutral case_step branch was accepted.", "The canonical case_step lifecycle regression reached prevention_locked with neutral_case_step_contract before the new promotion gate exposed that the prevention was still passive text."],
    scope: "universal",
    binding_scope: "action",
    action_binding: { tool_names: ["advanceHardeningIncident"] },
    enforcement_point: "Public neutral case_step schema, Step-2 action-rule binding, server-side neutral-to-semantic compilation, and continuous-hardening transition validation.",
    regression_test_id: "exposes and executes the complete neutral case_step contract through the normalized lifecycle",
    supersession_rule: "Any replacement must keep semantic hardening controls off the client surface while preserving complete server-side hardening transitions and pre-execution action binding.",
  },
  {
