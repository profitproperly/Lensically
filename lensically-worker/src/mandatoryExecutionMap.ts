...
export const WINNING_PATH_PROMOTIONS: readonly WinningPathPromotion[] = [
  ...
  },
  {
    ...
    winning_path: {
      ...
      procedure: ["Derive one neutral execution descriptor from the server-bound capability and actual tool effect at Step 3.", "Expose only the neutral action identity and read_only or mutation effect class to the client; keep raw handler names and arguments server-side.", "Require Step 4 to replay that exact descriptor with the opaque live-state token and reject any mismatch before execution.", "Generate descriptor schema and runtime validation from the same prepared-action contract.", "Bind failure history and root cause directly to the action, but keep the proven repair and enforcement procedure single-sourced in the linked prevention rule so competence growth cannot crowd required knowledge out of the payload budget.", "Remove retired profile_id plus inputs gateway wording from the Step-4 public description.", "Regression-test a harmless engineering read through the real five-stage lifecycle before closing the client-block incident."],
    },
  ...
  },
  {
    ...
    winning_path: {
      ...
      procedure: ["Expose the neutral case_step capability only.", "Use explicit ordinal stages a0 through a10 only; do not expose a generic next-state branch.", "Use a stage-specific closed typed contract so each stage accepts only evidence newly required at that state.", "Derive case_step_a0 through case_step_a10 as neutral Step-4 action identities from the immutable prepared stage so consecutive transitions remain client-distinguishable without exposing hardening semantics.", "Compile neutral fields server-side into advanceHardeningIncident semantics and carry previously stored evidence forward server-side.", "Bind this prevention to advanceHardeningIncident before Step 4 and require the normal hardening validator plus explicit lifecycle closure."],
    },
