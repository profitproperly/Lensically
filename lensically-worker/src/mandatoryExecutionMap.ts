...
export const WINNING_PATH_PROMOTIONS: readonly WinningPathPromotion[] = [
  ...
  },
  {
    ...
    losing_path: "Expose semantic hardening control-plane names, retain a generic next-state branch that collapses distinct transitions behind one client-visible mutation identity, allow premature transition evidence on a stage that does not require it, or allow the neutral case-step prevention to exist only as closure text without action binding.",
    root_cause: "The lifecycle exposed semantic hardening transition vocabulary and a cross-stage evidence envelope directly to the client, then retained generic stage n and one case_step mutation descriptor across distinct prepared transitions instead of compiling fully stage-specific neutral identities server-side.",
    winning_path: {
      surface: "main_gateway",
      route_intent: "advance hardening case",
      procedure: ["Expose the neutral case_step capability only.", "Use explicit ordinal stages a0 through a10 only; do not expose a generic next-state branch.", "Use a stage-specific closed typed contract so each stage accepts only evidence newly required at that state.", "Derive stable opaque Step-4 action identities exec_02_00 through exec_02_10 from the immutable prepared stage so consecutive transitions remain client-distinguishable without exposing hardening semantics.", "Compile neutral fields server-side into advanceHardeningIncident semantics and carry previously stored evidence forward server-side.", "Bind this prevention to advanceHardeningIncident before Step 4 and require the normal hardening validator plus explicit lifecycle closure."],
    },
    evidence: ["OpenAI pre-dispatch blocking previously rejected semantic hardening_transition forms while the neutral case_step branch was accepted.", "A later evidence-bearing a1 transition was client-blocked after the model supplied root-cause text before the classified state required it, proving optional cross-stage evidence remained an enforcement escape.", "In refreshed production, consecutive generic case_step mutation descriptors were blocked pre-dispatch while independent Recovery smoke proved the deployed server lifecycle healthy, proving generic next-state and generic Step-4 identity remained a client-compatibility escape."],
    scope: "universal",
    binding_scope: "action",
