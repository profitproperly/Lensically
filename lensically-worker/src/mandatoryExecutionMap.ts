...
export const WINNING_PATH_PROMOTIONS: readonly WinningPathPromotion[] = [
  ...
  },
  {
    ...
    evidence: ["OpenAI pre-dispatch blocking previously rejected semantic hardening_transition forms while the neutral case_step branch was accepted.", "A later evidence-bearing a1 transition was client-blocked after the model supplied root-cause text before the classified state required it, proving optional cross-stage evidence remained an enforcement escape.", "Stage-specific exec_02 identities improved dispatchability but a5 remained repeatedly blocked before Lensically receipt even with the minimum valid payload, proving that sharing the generic executeOperatorAction mutation surface remained an unresolved client-compatibility escape."],
    scope: "universal",
    binding_scope: "action",
    action_binding: { tool_names: ["advanceHardeningIncident"] },
    enforcement_point: "Public explicit a0-a10 case_step schema, Step-2 action-rule binding, Step-3 ordinal execution-descriptor derivation, dedicated executeOperatorCaseAction mutation schema, gateway/capability exact-match guard, server-side neutral-to-semantic compilation, and continuous-hardening transition validation.",
