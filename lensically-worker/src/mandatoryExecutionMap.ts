...
export const WINNING_PATH_PROMOTIONS: readonly WinningPathPromotion[] = [
  ...
  },
  {
    ...
    winning_path: {
      ...
      procedure: ["Expose the neutral case_step capability only.", "Use a stage-specific closed typed contract so each stage accepts only evidence newly required at that state.", "Compile neutral fields server-side into advanceHardeningIncident semantics and carry previously stored evidence forward server-side.", "Bind this prevention to advanceHardeningIncident before Step 4 and require the normal hardening validator plus explicit lifecycle closure."],
    },
