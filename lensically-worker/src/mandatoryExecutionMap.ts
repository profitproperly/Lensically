...
export const WINNING_PATH_PROMOTIONS: readonly WinningPathPromotion[] = [
  ...
  },
  {
    ...
    evidence: ["The old direct getEngineeringContinuation tool was explicitly read-only and operated successfully before lifecycle normalization.", "After normalization, get_engineering_continuation, hardening_status, and client_block_intake all reached Steps 2 and 3 but OpenAI blocked the generic executeOperatorAction call before Lensically dispatch.", "Independent Recovery server-to-server smoke completed the same deployed five-stage lifecycle, proving the underlying Lensically execution path remained functional."],
    scope: "universal",
    binding_scope: "action",
    action_binding: { tool_names: ["*"] },
