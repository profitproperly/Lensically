...
export const WINNING_PATH_PROMOTIONS: readonly WinningPathPromotion[] = [
  ...
  },
  {
    ...
    winning_path: {
      ...
      procedure: ["Derive one neutral execution descriptor from the server-bound capability and actual tool effect at Step 3.", "Expose only the neutral action identity and read_only or mutation effect class to the client; keep raw handler names and arguments server-side.", "Require Step 4 to replay that exact descriptor with the opaque live-state token and reject any mismatch before execution.", "Generate descriptor schema and runtime validation from the same prepared-action contract.", "Remove retired profile_id plus inputs gateway wording from the Step-4 public description.", "Regression-test a harmless engineering read through the real five-stage lifecycle before closing the client-block incident."],
    },
