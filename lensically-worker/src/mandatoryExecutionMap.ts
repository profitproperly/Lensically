...
export const WINNING_PATH_PROMOTIONS: readonly WinningPathPromotion[] = [
  ...
  },
  {
    ...
    winning_path: {
      ...
      procedure: ["Expose the neutral case_step capability only.", "Use explicit ordinal stages a0 through a10 only; do not expose a generic next-state branch.", "Use a stage-specific closed typed contract so each stage accepts only evidence newly required at that state.", "Derive stable opaque Step-4 action identities exec_02_00 through exec_02_10 from the immutable prepared stage so consecutive transitions remain client-distinguishable without exposing hardening semantics.", "Route mutating case_step descriptors only through the dedicated executeOperatorCaseAction public Step-4 gateway; keep case dry-runs on executeOperatorReadAction and reject case tokens on executeOperatorAction.", "Reject every non-case mutation token on executeOperatorCaseAction before execution.", "Compile neutral fields server-side into advanceHardeningIncident semantics and carry previously stored evidence forward server-side.", "Bind this prevention to advanceHardeningIncident before Step 4 and require the normal hardening validator plus explicit lifecycle closure."],
    },
