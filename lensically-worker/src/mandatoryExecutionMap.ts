...
export const WINNING_PATH_PROMOTIONS: readonly WinningPathPromotion[] = [
  ...
  },
  {
    ...
    winning_path: {
      ...
      procedure: ["Expose the neutral case_step capability only.", "Use explicit ordinal stages a0 through a10 only inside the Step-2 closed typed action contract; do not expose a generic next-state branch.", "Use a stage-specific closed typed contract so each stage accepts only evidence newly required at that state.", "Bind the exact stage, action identity, effect class, fingerprint, and arguments server-side in the signed Step-3 live-state token.", "For mutating case_step execution, expose only the dedicated executeOperatorCaseAction gateway with the opaque live-state token; do not require the client to replay the stage-specific action identity.", "Derive the exact case execution descriptor server-side from the verified token and reject every non-case mutation token before execution.", "Keep case dry-runs on executeOperatorReadAction and reject case tokens on executeOperatorAction.", "Compile neutral fields server-side into advanceHardeningIncident semantics and carry previously stored evidence forward server-side.", "Bind this prevention to advanceHardeningIncident before Step 4 and require the normal hardening validator plus explicit lifecycle closure."],
    },
    evidence: ["OpenAI pre-dispatch blocking previously rejected semantic hardening_transition forms while the neutral case_step branch was accepted.", "A later evidence-bearing a1 transition was client-blocked after the model supplied root-cause text before the classified state required it, proving optional cross-stage evidence remained an enforcement escape.", "Moving case mutations off the generic executeOperatorAction gateway allowed a5 and a6 to execute successfully through executeOperatorCaseAction.", "The immediately following validated a7 transition was blocked before Lensically receipt while using the same gateway; the stage-specific exec_02_07 descriptor was the only meaningful client-visible semantic delta, while the server-bound token already contained the exact stage and action identity."],
    scope: "universal",
    binding_scope: "action",
