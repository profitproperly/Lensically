  ...
  return plan.map((row) => {
    ...
    return {
      ...
      selection_evidence: {
        ...
        winner_initial_coverage_count: Number(receipt.winner_initial_coverage_count ?? 0),
        winner_proportional_weight: Number(receipt.winner_proportional_weight ?? 0),
        winner_exact_additional_share: Number(receipt.winner_exact_additional_share ?? 0),
        winner_rounded_additional_placements: Number(receipt.winner_rounded_additional_placements ?? 0),
        winner_final_target_count: Number(receipt.winner_final_target_count ?? 0),
        winner_actual_selected_count: Number(receipt.winner_actual_selected_count ?? 0),
        winner_target_satisfied: receipt.winner_target_satisfied === true,
        semantic_key: receipt.semantic_key ?? null,
        published_uses_72h: Number(receipt.published_uses_72h ?? 0),
        future_scheduled_uses: Number(receipt.future_scheduled_uses ?? 0),
        semantic_published_uses_24h: Number(receipt.semantic_published_uses_24h ?? 0),
        semantic_future_scheduled_uses: Number(receipt.semantic_future_scheduled_uses ?? 0),
        allocation_tier: receipt.allocation_tier ?? null,
        exposure_burden: Number(receipt.exposure_burden ?? 0),
        cooldown_hours: Number(receipt.cooldown_hours ?? 0),
        cooldown_relaxation: Number(receipt.cooldown_relaxation ?? 0),
