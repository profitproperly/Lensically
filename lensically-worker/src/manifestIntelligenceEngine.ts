...
export async function ensureManifestIntelligenceEngineTables(db: D1Database): Promise<void> {
  await Promise.all([
    assertDatabaseIntegrity(db, {
      table: "operator_manifest_semantic_signatures",
      columns: ["id", "brand_key", "content_type", "content_id", "scheduled_post_id", "published_post_id", "observed_at", "text_hash", "signature_version", "signature_json", "created_at", "updated_at"],
    }),
    assertDatabaseIntegrity(db, {
      table: "operator_manifest_maturity_evaluations",
      columns: ["id", "brand_key", "published_post_id", "checkpoint_hours", "evaluation_version", "evaluation_json", "structural_change_allowed", "created_at", "updated_at"],
    }),
    assertDatabaseIntegrity(db, {
      table: "operator_manifest_comparable_analyses",
      columns: ["id", "brand_key", "published_post_id", "checkpoint_hours", "analysis_version", "comparable_post_ids_json", "analysis_json", "created_at", "updated_at"],
    }),
    assertDatabaseIntegrity(db, {
      table: "operator_manifest_learning_observations",
      columns: ["id", "brand_key", "level", "feature_key", "checkpoint_hours", "sample_size", "supporting_count", "contradicting_count", "median_overall", "effect_size", "confidence_score", "confidence_label", "state", "evidence_json", "active", "learning_version", "created_at", "updated_at"],
    }),
    assertDatabaseIntegrity(db, {
      table: "operator_manifest_portfolio_states",
      columns: ["id", "brand_key", "family_key", "role", "recommended_role", "previous_role", "confidence_score", "confidence_label", "allocation_weight", "actual_decay", "reason", "evidence_json", "portfolio_version", "created_at", "updated_at"],
    }),
    assertDatabaseIntegrity(db, {
      table: "operator_manifest_state_transitions",
      columns: ["id", "transition_key", "brand_key", "entity_type", "entity_id", "from_state", "to_state", "reason", "evidence_json", "transitioned_at", "created_at"],
    }),
    assertDatabaseIntegrity(db, {
      table: "operator_manifest_experiments",
      columns: ["id", "brand_key", "experiment_key", "family_key", "hypothesis_json", "comparison_group_json", "maturity_windows_json", "result_criteria_json", "status", "latest_result_json", "follow_up_decision", "experiment_version", "created_at", "updated_at"],
    }),
    assertDatabaseIntegrity(db, {
      table: "operator_manifest_experiment_assignments",
      columns: ["id", "experiment_id", "brand_key", "cycle_id", "slot_key", "hypothesis_id", "scheduled_post_id", "published_post_id", "variant_key", "status", "created_at", "updated_at"],
    }),
  ]);
}
