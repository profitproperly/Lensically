...
- Step 2 `getOperatorKnowledge` loads task-relevant canonical knowledge nodes selected from the Step-1 tree. The model does not search/read this file directly for routine competency loading.
- Step 3 `getOperatorLiveState` loads task-relevant current-state scopes. Step 4 `executeOperatorAction` chooses the exact typed action only after that live state is known, computes the action's mandatory knowledge/state prerequisites server-side, and fails closed before execution if Steps 2-3 did not load them. Step 5 `closeOperatorAction` verifies evidence and checkpoints.
