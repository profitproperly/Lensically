...
- Step 2 `getOperatorKnowledge` accepts the same closed typed planned-action contract used by Step 4, derives exactly that action's canonical knowledge nodes, validates its arguments, fingerprints the action, and binds the resulting knowledge token to it. Callers do not choose node IDs. Runtime knowledge comes from deployment-local `lensically-worker/src/operatorKnowledgeRegistry.json`, so Step 2 requires no repository/network fetch; this handbook remains the human-maintenance companion and is never copied into startup.
