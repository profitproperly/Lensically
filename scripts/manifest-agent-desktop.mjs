...
function buildGenerationSlots(requestedCount) {
  const count = clampPostCount(requestedCount, MAX_POST_COUNT);
  return Array.from({ length: count }, (_, index) => `Post ${index + 1}`);
}
...
async function generateRun(postCount = DEFAULT_POST_COUNT) {
  ...
  const generationSlots = buildGenerationSlots(postCount);
...
async function moveRunPost(index, direction) {
  const latestRun = await loadLatestRun();
  if (!latestRun?.posts?.length) throw new Error("No generated run is available.");
  const numericIndex = Math.trunc(Number(index));
  if (!Number.isInteger(numericIndex) || numericIndex < 0 || numericIndex >= latestRun.posts.length) {
  ...
  if (delta === null) throw new Error("direction must be 'up' or 'down'.");
  const targetIndex = numericIndex + delta;
  if (targetIndex < 0 || targetIndex >= latestRun.posts.length) return latestRun;
  const [moved] = latestRun.posts.splice(numericIndex, 1);
...
async function movePost(index,direction){msg('Moving generated post.');try{const d=await api('/run/posts/move',{method:'POST',body:JSON.stringify({index,direction})});state.run=d.run;msg('Post moved.','ok')}catch(e){msg(e.message,'error')}finally{render()}}
...
async function handle(request, response) {
  try {
    ...
    if (url.pathname === "/run/posts/move" && request.method === "POST") {
      const body = await readBody(request);
      return send(response, 200, { ok: true, run: await moveRunPost(body.index, String(body.direction ?? "")) });
    }
